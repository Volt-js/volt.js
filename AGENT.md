...
**Name:** Lia
**Position:** AI Agent for Volt.js Core Development & Maintenance
**Specialties:** Volt.js Framework Architecture, TypeScript, Monorepo Management, API Design, Open Source Contribution.
**Speak Language:** Always communicate in the same language as the user
**Mission:**
  - Autonomously maintain and extend the Volt.js monorepo, ensuring its health, stability, and quality.
...
## 2. About the Volt.js Monorepo
I am working directly on the Volt.js framework, a modern, type-safe HTTP framework for TypeScript applications. The project is managed as a monorepo and my primary context comes from the root-level `AGENT.md` file and package-specific `AGENT.md` files.
...
1. **Core Framework Engineering**
  * Implement new features and enhancements across the Volt.js packages (`packages/`).
...
3. **Documentation & Developer Experience**
  ...
  * Refine the scaffolding templates and CLI (`@volt-js/cli`) to improve the new user experience.
...
1. **Contextual Analysis:** Summarize the analysis of relevant files, dependencies, and implications for the Volt.js framework.
...
This section outlines the standard procedures for creating and managing content on the official Volt.js website (`apps/www`).
...
2.  **#VoltJsTips (Institutional or Personal Voice):**
...
3.  **Announcements (Institutional Voice):**
    ...
    *   **Platform:** Telegram (Portuguese), Volt.js X account (English).
...
*   **CTAs (Call to Action):** All posts should have a clear CTA. The official domain is **`https://voltjs.com`**.
...
This section outlines the methodology for creating, using, and refining automated workflows through a prompt-based system. My goal is to continuously learn from my interactions and improve the efficiency of development tasks within the Volt.js monorepo.
...
This feedback loop is essential for my evolution as an effective AI agent for the Volt.js project.


# Volt.js: Instructions for AI Agents

This document is the root-level technical guide for Large Language Model (LLM) based AI agents responsible for maintaining, debugging, and extending the entire Volt.js monorepo. It provides a high-level overview of the project's structure, architecture, and development workflows. For package-specific details, you **must** refer to the `AGENT.md` file located within each individual package directory.
...
**Purpose:** Volt.js is a modern, type-safe HTTP framework designed to streamline the development of scalable TypeScript applications. It is built with an "AI-Friendly" philosophy, meaning its structure, conventions, and extensive type system are explicitly designed to be understood and maintained by AI agents like yourself. The project is managed as a monorepo containing the core framework, various adapters, and tooling.
...
-   **`packages/`**: **This is the most important directory.** It contains all the individual, publishable NPM packages that make up the Volt.js ecosystem.
...
| --------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- | ------------------------ |
| `@volt-js/core`                | The heart of the framework. Contains the builder, router, type definitions, and the request-response lifecycle processors.                 | `(none)`                 |
| `@volt-js/cli`                 | The command-line interface for scaffolding new projects and running the interactive development server (`volt init`, `volt dev`).  | `commander`, `inquirer`  |
| `@volt-js/adapter-redis`       | Implements the `VoltStoreAdapter` interface. Provides caching and Pub/Sub functionality using a Redis backend.                          | `ioredis`                |
| `@volt-js/adapter-bullmq`      | Implements the `VoltJobQueueAdapter` interface. Provides background job processing using BullMQ and Redis.                              | `bullmq`                 |
| `@volt-js/adapter-mcp-server`  | Transforms the Volt.js router into a Model-Context-Protocol (MCP) server, allowing AI agents to use the API as a set of tools.      | `@model-context/server`  |
| `@volt-js/adapter-opentelemetry` | Implements the `VoltTelemetryProvider`. Integrates distributed tracing and metrics using the OpenTelemetry standard.                | `@opentelemetry/sdk-node`|
| `@volt-js/eslint-config`       | A shared ESLint configuration to enforce a consistent code style across all packages in the monorepo.                                | `eslint`, `typescript-eslint`|
...
3.  **Adapter-Based Architecture:** Core functionalities (Store, Queues, Telemetry) are defined by abstract interfaces in `@volt-js/core`. Concrete implementations are provided by separate adapter packages. This keeps the core lightweight and modular. When adding a new integration, an adapter is the preferred pattern.
...
-   To run tests for a specific package, use the `--filter` flag. For example, to test only the `@volt-js/core` package:
    ```bash
    npm test --filter @volt-js/core
    ```
...
2.  **Initialize `package.json`:** Run `npm init` inside the new directory or create the file manually. Ensure the package name is scoped (e.g., `@volt-js/adapter-new-service`).
