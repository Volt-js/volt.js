# AI Agent Maintenance Manual: Volt.js Starter (Next.js)

This document provides a technical guide for Large Language Model (LLM) based AI agents responsible for maintaining, debugging, and extending the **Volt.js Starter for Next.js**.
...
### 1.1. Name
Volt.js Starter: Next.js Full-Stack App

### 1.2. Purpose
This project is a starter template for building high-performance, full-stack, type-safe web applications using **Next.js**. It leverages **React Server Components (RSC)** for optimal performance and **Volt.js** for a robust, type-safe API layer that works seamlessly across server and client environments.
...
**Name:** Lia
**Position:** Code Agent for Volt.js Core Development & Maintenance
**Specialties:** Volt.js, TypeScript, Next.js, Product Development, UI/UX Design, Growth Marketing and Software Development.
...
**Mission:**
  ...
  - Guide developers in creating robust, scalable projects using the Volt.js Starter for Next.js.
...
This application utilizes Next.js's App Router to run code on both the server (RSCs, API Routes) and the client (Client Components). Volt.js is integrated as a structured API layer within the Next.js project.

### 2.1. API Entry Point: The Bridge
The primary entry point for all API requests is a single Next.js Route Handler:
-   **File**: `app/api/[[...volt]]/route.ts`
-   **Mechanism**: This file uses `nextRouteHandlerAdapter` to translate incoming Next.js requests into a format Volt.js understands, and vice-versa for responses. It exports handlers for all HTTP methods (`GET`, `POST`, etc.). All requests matching `/api/*` are directed here. You will rarely need to modify this file.

### 2.2. Volt.js API Layer: Feature-Based Structure
All backend business logic resides within the `src/features` directory, following a **Feature-Sliced Architecture**.
-   `src/volt.ts`: The central configuration file where the `volt` instance is created and global adapters/plugins are registered.
-   `src/volt.router.ts`: Assembles the main `AppRouter` by importing and registering all feature controllers. **This file is the single source of truth for your API's structure.**
...
The file `src/volt.client.ts` defines the **type-safe Volt.js API client**. This is the primary way the frontend interacts with the backend.
-   **Auto-Generated**: This file **MUST NOT be edited manually**. It is a build artifact that perfectly mirrors the API defined in `volt.router.ts`. It is regenerated automatically on change by the dev server.
...
## 3. Backend Deep Dive: Core Volt.js Concepts

### 3.1. The Volt Builder (`volt.ts`)
...
export const volt = Volt
  ...
  // 5. Enable the OpenAPI documentation and Volt Studio
  .docs({
    openapi,
  })
  ...
  .config({
    ...
    // You can add any more config and access before with volt.config.[any_property] (Fully type-safety)
  })
...
-   **Base Context (`src/volt.context.ts`)**: Defines global services, like the database client, available in every request.
...
export const authProcedure = volt.procedure({
...
-   **Controller (`volt.controller`)**: An organizational unit that groups related actions under a common base `path`.
-   **Action**: A single API endpoint.
    -   `volt.query()`: For `GET` requests (fetching data).
    -   `volt.mutation()`: For `POST`, `PUT`, `PATCH`, `DELETE` requests (changing data).

### 3.4. Validation
Volt.js uses a two-layer validation approach.
...
Volt.js has two types of actions: `volt.query()` for data fetching (GET) and `volt.mutation()` for data modification (POST, PUT, DELETE). They share some properties but have key differences.

##### Query Action (`volt.query`)
...
| query | object | No | A Zod schema to validate URL query parameters |
| use | array | No | An array of middleware to run before the handler |
| handler | (context: VoltContext) => void | Yes | The function containing your logic |

#### Handler Context Properties (VoltContext)
The handler function presents on Actions and Procedures receives a VoltContext object with the following properties:
...
| response | VoltResponse | Response utilities with methods like `.created()`, `.ok()`, `.revalidate()` |
| realtime | VoltRealtime | Services for triggering real-time updates to connected clients |
...
2.  **Enqueue a Job**: From a mutation or other backend logic, use `volt.jobs.<namespace>.schedule()` to add a job to the queue. The API responds to the user immediately while the job runs in the background.

```typescript
// Enqueuing a job from a mutation
await volt.jobs.emails.schedule({
...
-   **Caching**: Use `volt.store.get()` and `volt.store.set()` to implement caching patterns like cache-aside, reducing database load.
-   **Pub/Sub**: Use `volt.store.publish()` and `volt.store.subscribe()` for event-driven communication between different parts of your application.
...
1.  **Automatic Revalidation (The "Magic")**:
    ...
    ```typescript
    // Backend Controller
    export const postsController = volt.controller({
      actions: {
        list: volt.query({
...
2.  **Scoped Revalidation**: To target specific users, define scopes on the `<VoltProvider>` using `getScopesIds` (e.g., `['user:123']`) and pass a function to `.revalidate()` on the backend to specify target scopes: `.revalidate(['query_key'], (ctx) => [\`user:${ctx.user.id}\`])`.

---
## 5. Client-Side Deep Dive

### 5.1. `<VoltProvider>`
This is a **mandatory** root provider (`src/providers/volt.provider.tsx`) for all client-side functionality. It manages the query cache and the real-time connection for live updates (SSE/WebSocket, depending on adapter).
...
### 6.2. `volt dev`
The `volt dev` command (aliased to `npm run dev`) is your primary development tool. It runs and manages all processes (Next.js, Volt watcher) in a unified dashboard.

### 6.3. Schema and Documentation Generation
Volt.js provides several commands to generate TypeScript types and API documentation:

1. `volt generate schema` - Creates or updates `src/volt.schema.ts` with TypeScript types for the frontend client.
2. `volt generate docs` - Generates OpenAPI specification. Use the `--ui` flag to also generate HTML documentation.
3. `volt dev --docs` - Starts the development server with automatic schema and OpenAPI generation on file changes. This also provides a playground at the `/docs` path of your API.

To find the API URL for the Volt Studio playground, check the `src/volt.ts` file for the `baseURL` and `basePath` configuration. The Volt Studio is available at `{baseURL}{basePath}/docs` for testing the API.
...
2.  **Consult Configuration**: If the port is not `3000`, review `src/volt.ts` for a `baseURL` in the `.config()` block, which may specify a different port.
...
1.  **Locate OpenAPI Spec**: Find the path to the OpenAPI specification file by reading `src/volt.ts` and looking for the `docs.openapi` configuration. This will point to the JSON file (e.g., `src/docs/openapi.json`).
...
2.  **DO NOT EDIT Generated Files**: Specifically, `src/volt.client.ts` and `src/volt.schema.ts` are build artifacts. Modify backend controllers to change the client.
...
6.  **Use Client Components for Interactivity**: Use Client Components (`'use client'`) for any UI that involves user interaction, state, or lifecycle effects. This is where you will use the Volt.js hooks.

---

## 9. External Documentation Links

For more detailed information on Volt.js concepts, refer to the official documentation.

-   **[Volt.js Official Website](https://voltjs.com)**: The official website for Volt.js framework with comprehensive documentation.

-   **[LLMs Documentation](https://voltjs.com/llms.txt)**: Optimized documentation specifically for AI assistants like Lia. This is the single source of truth for the most up-to-date Volt.js documentation when working with LLMs. Always check this resource during tasks to ensure you're using the most recent APIs and features, as Volt.js is frequently updated with new functionality.

-   **[Volt Studio Documentation](https://voltjs.com/docs/studio)**: Documentation for the Volt Studio, a web-based playground for testing and exploring your API. The Studio is automatically available at the `/docs` path of your API when running with `volt dev --docs`.
...
-   **Core Concepts**: The starting point for understanding the framework's architecture.
    -   **The Volt Builder**: Explains how `src/volt.ts` uses a fluent API (`.context()`, `.logger()`, `.create()`) to build the application instance.
...
-   **React Client Integration**: Essential reading for frontend work.
    -   **API Client**: Explains `src/volt.client.ts`.
    -   **VoltProvider**: A mandatory React provider.
