...
export const volt = Volt
  ...
  .telemetry(telemetry)
  .config({
    baseURL: process.env.BUN_PUBLIC_VOLT_API_URL || 'http://localhost:3000',
    basePATH: process.env.BUN_PUBLIC_VOLT_API_BASE_PATH || '/api/v1',
  })
  .docs({
    openapi,
    info: {
      title: 'Volt.js Starter (Bun REST API)',
      version: '1.0.0',
      description: 'A sample Bun REST API built with Volt.js',
    }
  })
  .create()
