...
export const auth = volt.procedure({
...
await volt.jobs.emails.enqueue({
  ...
  actions: {
    createUser: volt.mutate({
      ...
      handler: async ({ input, context }) => {
        ...
        // Publish an event via Volt Store (backed by Redis)
        ...
        await volt.store.publish('user.created', {
          id: user.id,
          name: user.name,
          email: user.email,
...
volt.store.subscribe('user.created', async (data) => {
...
volt.store.subscribe('user.*', (data, channel) => {
  ...
  actions: {
    ...
    update: volt.muate({
      ...
      handler: async ({ request, response, context, query }) => {
        ...
        // Invalidate cache
        await volt.store.del(\`user:\${input.id}\`);
