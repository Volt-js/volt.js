...
The best practice is to **invalidate** the relevant queries in the `onSuccess` callback. This tells Volt.js to automatically refetch that data, ensuring your UI always reflects the latest state.
...
import { api, useQueryClient } from '@/volt.client';
...
If you are using **Volt.js Realtime**, you can often skip manual invalidation and use server-side `.revalidate()` for a more powerful, automated approach.
