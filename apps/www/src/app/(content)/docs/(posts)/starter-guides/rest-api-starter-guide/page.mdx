# Guide: Building High-Performance, Type-Safe REST APIs with Volt.js

Welcome to the definitive guide for building backend services with the Volt.js REST API starters. This document provides a comprehensive walkthrough for creating high-performance, scalable, and fully type-safe REST APIs.

Volt.js offers several starter templates for building headless backend services, allowing you to choose the runtime and ecosystem that best fits your needs without compromising on architecture or developer experience. This guide covers the three primary REST API starters:
...
While each starter uses a different underlying runtime, their core architecture and development workflow within Volt.js are **nearly identical**. This guide will walk you through the common patterns and highlight the specific differences where they matter.
...
The core philosophy is to provide a clean, structured, and scalable architecture for your business logic. Instead of placing all your logic in a single file or a flat directory of route handlers, Volt.js promotes a **feature-based architecture**. Each distinct part of your application (e.g., "users," "invoices," "products") lives in its own self-contained feature directory. This makes the codebase easy to navigate, maintain, and scale.

The runtime (Express, Bun, or Deno) acts as a **thin HTTP layer**. Its only job is to receive an incoming HTTP request and pass it along to the Volt.js engine for processing. All the complex work—routing, validation, middleware, and business logic—is handled by Volt.js.

### 1.2. Type Safety for Your Consumers
The most powerful feature of a headless Volt.js API is the type safety it provides to its **consumers**. An API is useless without clients (a web app, a mobile app, another microservice). Volt.js ensures that the contract between your API and its clients is never broken.

It achieves this by automatically generating two critical artifacts:
-   `src/volt.schema.ts`: A JSON schema representation of your entire API router.
-   `src/volt.client.ts`: A fully-typed TypeScript client that can be used to call your API.
...
1.  **Initialize the Project**: Use the Volt.js CLI to scaffold your new API project.
    ```bash
...
This file is where the underlying runtime is configured to hand off requests to Volt.js.

**Express REST API Starter (`src/index.ts`)**
The Express starter uses the `expressAdapter`. It creates a standard Express app and mounts the Volt.js router as a middleware. This is a classic and highly robust pattern.
...
// All requests to /api/v1/* are handled by Volt.js
...
The Bun starter uses the high-performance native `Bun.serve` API. It inspects the request URL and forwards API traffic to the Volt.js handler.
...
const server = Bun.serve({
  ...
  async fetch(request) {
    ...
    if (url.pathname.startsWith('/api/v1')) {
      // Let Volt.js handle the request
...
import { AppRouter } from '@/volt.router.ts'; // Note the .ts extension
...
-   `src/volt.ts`: Where the core `volt` instance is built and configured.
-   `src/volt.router.ts`: Where all feature controllers are imported and assembled into the main `AppRouter`.
-   `src/features/`: The home for all your business logic, organized by feature.

This consistency means you can learn the Volt.js patterns once and apply them anywhere, regardless of your preferred JavaScript runtime.
...
```bash
npx @volt-js/cli generate feature snippets --schema prisma:Snippet
```
This generates a full CRUD API for snippets inside `src/features/snippets/`.

### Step 4: Register the Controller
Open `src/volt.router.ts` and register the new controller.
...
### 5.1. Background Jobs with Volt Queues
...
### 5.2. Caching with the Volt Store (Redis)
...
let snippet = await volt.store.get<Snippet>(cacheKey);
...
if (!snippet) {
  ...
  if (snippet) {
    ...
    await volt.store.set(cacheKey, snippet, { ttl: 3600 });
  }
}
...
-   `dist/volt.client.mjs`
-   `dist/volt.schema.json`
-   `dist/volt.client.d.ts` (the type definitions)
