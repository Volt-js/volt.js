# Full-Stack Guide: Building with the Volt.js TanStack Start Starter

Welcome to the definitive guide for the Volt.js TanStack Start starter. This document will walk you through building a bleeding-edge, full-stack, end-to-end type-safe application. We will explore the project's architecture, which combines the Vite-powered speed of TanStack Start with the structured, type-safe API layer of Volt.js.
...
-   **TanStack Query**: The gold standard for data fetching in React. It handles caching, revalidation, and server-state management, and it's what powers the Volt.js client hooks.

### 1.2. Volt.js: The Structured, Type-Safe API Layer
Volt.js integrates into this ecosystem as the dedicated API layer. It provides a clean, feature-based architecture for your backend logic, which lives right inside your TanStack Start project.

The synergy is powerful:
-   **End-to-End Type Safety**: Volt.js introspects your API controllers and auto-generates a client. TanStack Router is also fully type-safe. This means you have a continuous chain of type safety from your database schema, through your API layer, through your router, and into your React components.
...
-   **Ultimate Developer Experience**: You get Vite's speed, TanStack's powerful tools, and Volt.js's structured, safe, and feature-rich backend capabilities (like built-in Redis caching, background jobs, and real-time updates).
...
1.  **Initialize the Project**: Use the Volt.js CLI to scaffold a new TanStack Start project.
    ```bash
    npx @volt.js/cli init my-tanstack-app
    ```
...
5.  **Run the Dev Server**:
    ...
    npm run dev
    ```
    This single command starts the Vite development server. Vite is responsible for both serving your frontend and handling the API requests, which it delegates to Volt.js. You will see Vite's familiar, speedy output in your terminal.
...
│   ├── volt.client.ts           # << Auto-generated Type-Safe Client
...
-   `src/routes/__root.tsx`: This is the root layout component for your entire application. It's where you'll find the main `<html>` and `<body>` tags, and it's where the **`VoltProvider`** is set up. This provider is essential for the client-side hooks (`useQuery`, `useMutation`) to work correctly.
-   `src/routes/api/v1/$.ts`: This is the critical bridge between TanStack Start and Volt.js. It's a **catch-all API route**. The `$` in the filename tells TanStack Router to match any path under `/api/v1/`.
...
-   The `volt.*.ts` and `features/` directories serve the exact same purpose as in the Next.js starter: they define your backend API's structure and logic.
...
Use the `volt generate` command to create all the necessary backend files.
```bash
npx @volt.js/cli generate feature tasks --schema prisma:Task
```
This command creates the controller, procedures, and Zod interfaces for your `Task` feature inside `src/features/tasks/`.

### Step 4: Register the Controller
Open `src/volt.router.ts` and register the new `tasksController`.
...
```
When you save this file, Vite's dev server will automatically detect the change, and Volt.js will regenerate `volt.client.ts` in the background. `api.tasks` is now available on your client.
...
We'll now enable Volt.js's automatic real-time feature to make our task list update instantly.
...
You have successfully built a fast, modern, and fully type-safe full-stack application using TanStack Start and Volt.js.

We've covered:
-   The core principles of combining Vite, TanStack tools, and Volt.js.
...
-   Building a reactive frontend with TanStack Router and the auto-generated Volt.js client hooks.
