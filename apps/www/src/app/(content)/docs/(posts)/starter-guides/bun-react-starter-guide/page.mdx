# Full-Stack Guide: Building a High-Performance SPA with Bun, React, and Volt.js

Welcome to the comprehensive guide for the Volt.js Bun + React Starter. This document will take you on a journey to build an ultra-fast, modern, and fully type-safe Single Page Application (SPA). We'll harness the incredible speed of Bun as our server, runtime, and bundler, and pair it with the robust, type-safe API capabilities of Volt.js.
...
-   **Volt.js** provides the entire backend API layer, bringing structure, scalability, and its signature end-to-end type safety to the project.

### 1.3. End-to-End Type Safety
Just like in other Volt.js starters, this is a non-negotiable feature. Volt.js generates a type-safe client based on your API controllers. Your React application imports this client, giving you full IntelliSense and compile-time guarantees that your frontend and backend are always in sync.
...
1.  **Initialize the Project**: Use the Volt.js CLI to scaffold a new project.
    ```bash
    npx @volt-js/cli init my-bun-app
    ```
...
5.  **Run the Dev Server**:
    ...
    bun run dev
    ```
    This command starts the `volt dev` process, which in turn runs the Bun server with file-watching and hot-reloading enabled for both the backend and the React frontend.
...
-   **`src/index.tsx`**: This is the most unique file in this starter. It acts as the **unified entry point for both the server and the client**.
    -   When run by Bun (on the server), it executes the `Bun.serve` block, which starts the HTTP server. This server inspects incoming request URLs. If the URL starts with `/api/v1/`, it passes the request to the Volt.js router. Otherwise, it serves the `public/index.html` file.
...
-   **`volt.ts`, `volt.router.ts`, `features/`**: These form the core of your backend, responsible for configuring Volt.js, defining the API's shape, and housing all your business logic.
-   **`volt.client.ts`**: The auto-generated, type-safe client that provides the React hooks (`.useQuery()`, `.useMutation()`) your SPA will use to communicate with the backend.
...
Use the `volt generate` command to create the backend files automatically.
```bash
bunx @volt-js/cli generate feature journalEntries --schema prisma:JournalEntry
```
...
Open `src/volt.router.ts` and register the new `journalEntriesController`.
...
```
When you save this, the dev server will regenerate `volt.client.ts`. The `api.journalEntries` client is now ready to be used by your React app.
    ...
    <div className="min-h-screen bg-gray-50">
      <main className="container mx-auto p-4 sm:p-8">
        <header className="text-center mb-8">
          ...
          <p className="text-lg text-gray-600">A simple journal built with Bun, React, and Volt.js</p>
        </header>
        <CreateEntryForm />
        <JournalFeed />
      </main>
...
Volt.js makes real-time functionality incredibly simple.
...
In the same file, chain `.revalidate()` to the `create` mutation's response. This tells Volt.js which live query to update.
...
You have now built a high-performance, full-stack, type-safe Single Page Application using a truly modern toolchain. The combination of Bun's speed, React's component model, and Volt.js's structured, safe API layer creates a development experience that is both productive and enjoyable.
...
-   Using the Volt.js CLI to rapidly scaffold a complete backend feature from a database schema.
