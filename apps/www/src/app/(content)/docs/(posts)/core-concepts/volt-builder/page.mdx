# The Volt Builder: Your Application's Foundation

Every Volt.js application begins with the **Volt Builder**. It is a fluent, chainable API designed to guide you through the process of composing and configuring your application's core components in a structured and fully type-safe manner.
...
The Volt Builder is designed around five core principles:
...
The Volt Builder uses a fluent interface pattern where each method returns a new builder instance with updated type information. This ensures that TypeScript can provide accurate autocompletion and type checking throughout your configuration process.
...
<Accordion title=".context<T>() - Define application context type">
  ...
  const volt = Volt.context<AppContext>();
  ```
...
<Accordion title=".middleware(middlewares) - Configure global middleware">
  <Field name="middlewares" type="readonly VoltProcedure<any, any, any>[]" required description="An array of middleware procedures to apply globally." />
...
<Accordion title=".config(routerConfig) - Set base URL and path">
  <Field name="routerConfig" type="TConfig extends VoltBaseConfig" required description="Configuration object containing base URL and path settings." />
...
<Accordion title=".store(storeAdapter) - Configure caching and storage">
  <Field name="storeAdapter" type="VoltStoreAdapter" required description="A store adapter implementing the VoltStoreAdapter interface." />

  ```typescript
  import { RedisAdapter } from '@volt.js/adapter-redis';
  ...
  **Supported Adapters:**
  - `@volt.js/adapter-redis` - Redis-based storage with pub/sub support
  - Custom adapters implementing `VoltStoreAdapter` interface
...
<Accordion title=".logger(loggerAdapter) - Set up application logging">
  <Field name="loggerAdapter" type="VoltLogger" required description="A logger adapter implementing the VoltLogger interface." />

  ```typescript
  import { ConsoleLogger } from '@volt.js/logger-console';
  ...
  **Custom Loggers:**
  Implement the `VoltLogger` interface to create custom logging solutions.
</Accordion>
...
<Accordion title=".jobs(jobsAdapter) - Enable background job processing">
  ...
  ```typescript
  import { BullMQAdapter } from '@volt.js/adapter-bullmq';
  ...
  **Supported Adapters:**
  - `@volt.js/adapter-bullmq` - BullMQ integration with Redis
...
<Accordion title=".telemetry(telemetryProvider) - Add observability and monitoring">
  <Field name="telemetryProvider" type="VoltTelemetryProvider" required description="A telemetry provider implementing distributed tracing and metrics collection." />

  ```typescript
  import { OpenTelemetryProvider } from '@volt.js/adapter-opentelemetry';
  ...
  **Supported Providers:**
  - `@volt.js/adapter-opentelemetry` - OpenTelemetry integration
...
<Accordion title=".plugins(pluginsRecord) - Extend framework functionality">
  <Field name="pluginsRecord" type="Record<string, VoltPlugin>" required description="An object containing plugin instances keyed by their names." />

  ```typescript
  import { AuthPlugin } from '@volt.js/plugin-auth';
  import { CachePlugin } from '@volt.js/plugin-cache';
  ...
  const volt = Volt
    ...
    .docs({
      ...
      info: {
        title: 'My API',
        version: '1.0.0',
        description: 'A comprehensive API built with Volt.js'
      },
      servers: [
        {
...
<Accordion title=".create() - Build the final Volt instance">
  Creates the final Volt instance with all configured adapters and services.
...
Now that you have your Volt instance configured, you can start building your application:
