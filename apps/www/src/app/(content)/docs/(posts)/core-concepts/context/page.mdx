...
Unlike the context in some other frameworks which is often a simple, static object, the context in Volt.js is **dynamic and composable**. It starts with a base shape and is progressively enriched by middleware (Procedures), creating a tailored, fully-typed context for each specific action.

## 1. The Base Application Context (`AppContext`)

Everything starts with the base context. This is the global state that should be available to your entire application. You define its shape and creation logic in `src/volt.context.ts`.
 ...
 */
export type VoltAppContext = ReturnType<typeof createVoltAppContext>;
```

This `VoltAppContext` type is then passed to the Volt Builder in `src/volt.ts` to set the foundation for your application's type system:
...
export const volt = Volt
  .context<VoltAppContext>() // Setting the base context type
...
Because we defined `database` in our `VoltAppContext`, TypeScript provides full autocompletion and type-checking for `context.database`.

## 3. The Magic: A Dynamic, Extendable Context

Here is where Volt.js truly shines. The context passed to your action handler is not just the base context; it's a **merged object** composed of the base context **plus** any data returned by the procedures (middleware) used in that action.
...
Notice that we didn't have to manually tell TypeScript that `context.user` exists. Volt.js infers this automatically from the `use: [auth]` array. The final context for the `getProfile` handler is a merged type of `VoltAppContext & { user: { id: string; email: string; } }`.
...
1. **Base Context:** The request starts with the global `VoltAppContext` (e.g., `{ database }`).
