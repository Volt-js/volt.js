...
Every procedure receives an `VoltProcedureContext` object that contains all the information about the current request:

### Understanding the Procedure Context: A Properties Breakdown

To understand the `VoltProcedureContext` object that every procedure receives, you can expand the section below to see a detailed breakdown of its properties.

<Accordion title="VoltProcedureContext Properties">
...
Let's start with a simple example using the `volt.procedure` function. We'll create a request logging procedure:

```typescript
import { volt } from '@/volt';

// Create a simple logging procedure
export const requestLogger = volt.procedure({
  name: 'RequestLogger',
...
<Accordion title="Future Enhancements">
  <Field name="Router and Controller Level Procedures" type="roadmap">
    Currently, procedures can only be used at the action level or builder level. Support for router and controller level procedures is planned for future releases. You can track this feature request and vote for it on [GitHub Issue #13](https://github.com/andeerc/volt-js/issues/13).
  </Field>
</Accordion>
...
Following established best practices when creating procedures ensures your code remains maintainable, reusable, and type-safe. These guidelines will help you build robust middleware that integrates seamlessly with the Volt.js ecosystem.
...
<Accordion title="Keep Procedures Focused">
  ...
  export const loggingProcedure = volt.procedure({
    name: 'Logging',
    handler: async ({ request, response, next }) => {
  ...
  export const authAndLoggingProcedure = volt.procedure({
    name: 'AuthAndLogging',
    handler: async ({ request, response, next }) => {
...
<Accordion title="Use Descriptive Names">
  ...
  export const jwtAuthenticationProcedure = volt.procedure({
    name: 'JWTAuthentication',
    handler: async ({ next }) => next(),
  });
  
  export const requestRateLimitProcedure = volt.procedure({
    name: 'RequestRateLimit',
    handler: async ({ next }) => next(),
  });
  
  export const inputValidationProcedure = volt.procedure({
    name: 'InputValidation',
    handler: async ({ next }) => next(),
  });

  // Avoid - Vague and unclear
  export const authProc = volt.procedure({
    name: 'Auth',
    handler: async ({ next }) => next(),
  });
  
  export const middleware1 = volt.procedure({
    name: 'Middleware1',
    handler: async ({ next }) => next(),
  });
  ```
...
<Accordion title="Handle Errors Gracefully">
  ...
  ```typescript
  export const safeProcedure = volt.procedure({
    name: 'SafeProcedure',
    handler: async ({ request, response, next }) => {
      try {
...
<Accordion title="Use Schema Validation for Configuration">
  ...
  export const configurableProcedure = volt.procedure({
...
<Accordion title="Document Your Procedures">
   ...
   * ```
   */
  export const rateLimitProcedure = volt.procedure({
    name: 'RateLimit',
    schema: z.object({
...
Procedures are a fundamental building block of Volt.js applications, providing a powerful and flexible way to implement cross-cutting concerns in your API using the `volt.procedure` function. By understanding how to create, compose, and use procedures effectively, you can build more maintainable, reusable, and type-safe applications.
...
<Accordion title="Next Steps">
  <Field name="Ready to Build" type="next-step">
    With this comprehensive understanding of procedures, you're ready to build sophisticated, production-ready APIs with Volt.js. Explore [Controllers and Actions](/docs/core-concepts/controllers) and [Request and Response Handling](/docs/core-concepts/request-response) to complete your knowledge.
  </Field>
</Accordion>