...
Welcome to the Volt.js Quick Start Guide! This tutorial provides a detailed, step-by-step walkthrough to build your first fully type-safe API endpoint. We'll go from an empty directory to a running server, explaining each concept along the way.
...
## Step 1: Create Your Volt.js Project

We'll start by using the official `volt init` command, which scaffolds a new, production-ready project with a logical folder structure and all necessary configurations.
...
<CodeGroup>
  <Code title="npm">
  ```bash
  npx @volt.js/cli init my-first-api
  ```
  </Code>
  <Code title="pnpm">
  ```bash
  pnpm dlx @volt.js/cli init my-first-api
  ```
  </Code>
  <Code title="bun">
  ```bash
  bunx @volt.js/cli init my-first-api
  ```
  </Code>
</CodeGroup>
...
<Callout type="info" title="What did the CLI just do?">
  The `volt init` command created a starter project that includes the Volt.js core, essential configuration files (`volt.ts`, `volt.router.ts`), and a logical, feature-based directory structure under `src/`. This setup is designed for scalability and maintainability.
</Callout>
...
In Volt.js, a `Controller` is a file that groups related API endpoints. These endpoints are called `Actions` (either a `Query` for GET requests or a `Mutation` for POST, PUT, DELETE, etc.).
...
export const greetingController = volt.controller({
...
<Accordion title="volt.controller Properties">
...
Open `src/volt.router.ts` and modify it to include your new controller:
...
<Callout type="info" title="What is the AppRouter?">
  The `AppRouter` is the heart of your application's API. It aggregates all your controllers and defines the overall shape of your API. By exporting its *type*, you enable the Volt.js client to have fully type-safe access to your backend.
</Callout>

## Step 4: Run the Development Server

Volt.js includes an interactive development server that provides real-time feedback and a dashboard for your API.
...
This command executes `volt dev --interactive`. You should see a dashboard in your terminal, confirming that the server is running successfully on `http://localhost:3000`.
...
curl "http://localhost:3000/api/v1/greetings/hello?name=Volt"
```
...
You have successfully built and tested your first fully type-safe API endpoint with Volt.js!

In this guide, you have learned how to:
-   **Scaffold a project** using `volt init`.
...
*   **[Core Concepts](/docs/core-concepts/the-volt-builder)**: Take a deep dive into the fundamental building blocks of the framework.
