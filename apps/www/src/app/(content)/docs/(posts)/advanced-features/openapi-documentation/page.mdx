...
Volt.js provides automatic OpenAPI spec generation and an optional interactive UI to explore your API. This page explains how to configure, generate, and serve documentation, and how to customize metadata and security.

## Overview

- Generate `openapi.json` via CLI: `volt generate docs`.
- Serve Volt Studio (Scalar API Reference) in development with `volt dev --docs`, or in production via the Core `playground` config.
- Customize metadata via the `.docs()` method in your Volt builder (`info`, `servers`, `securitySchemes`, and `playground`).
...
```bash
npx @volt-js/cli generate docs --output ./docs/openapi.json
```

- Use the `--ui` flag to also generate an HTML page with Scalar API Reference UI:

```bash
npx @volt-js/cli generate docs --output ./docs --ui
```

This will create `./docs/openapi.json` and `./docs/index.html`.

## Serve documentation with Volt Studio

During development, you can enable Volt Studio on your local server with:

```bash
npx @volt-js/cli dev --docs --docs-output ./docs
```
...
## Configure via Volt Builder

You can configure documentation metadata and security using the `.docs()` method in your Volt builder:
...
<Accordion title="Router Not Found Error">
  When using `volt generate docs`, ensure your router is properly exported from your application entry point.
</Accordion>
...
<Accordion title="Studio Disabled in Production Environment">
  To enable Volt Studio in production, configure the `playground.security` option in your `DocsConfig` to implement proper access controls.
</Accordion>