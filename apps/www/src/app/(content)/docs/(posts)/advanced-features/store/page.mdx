# Volt.js Store: High-Performance Caching and Messaging
...
**Volt.js Store** is a powerful, integrated service that addresses both of these needs. It provides a unified, driver-based API for:
...
Like other Volt.js systems, the Store is built on a modular architecture. The officially recommended driver is the **Redis Store Adapter**, which leverages the speed and power of Redis.

---

## 1. Setup and Configuration

To use the Store, you first need to install the necessary dependencies, create the adapter instance, and register it with the Volt Builder.
...
Finally, enable the Store feature in `src/volt.ts` by passing your adapter to the `.store()` method.
...
export const volt = Volt
  ...
  .create();
```
With this configuration, the `volt.store` object becomes available throughout your application for direct use, and a `store` property is added to the `context` within your actions and procedures.
...
getProfile: volt.query({
  ...
  handler: async ({ request, context, response }) => {
    ...
    const cachedUser = await volt.store.get<User>(cacheKey);

    if (cachedUser) {
      volt.logger.info(`Cache HIT for key: ${cacheKey}`);
      return response.success(cachedUser);
    }

    volt.logger.info(`Cache MISS for key: ${cacheKey}`);
    ...
    await volt.store.set(cacheKey, user, { ttl: 3600 });
...
updateUserRole: volt.mutation({
  ...
  handler: async ({ request, context, response }) => {
    ...
    await volt.store.publish('user-events', {
      type: 'ROLE_UPDATED',
      payload: { userId: id },
    });
...
export function initializeEventListeners() {
  ...
  volt.store.subscribe('user-events', (message) => {
...
- [Volt.js Realtime](/docs/advanced-features/realtime) - Discover real-time features
- [Volt.js Plugins](/docs/advanced-features/plugins) - Explore the plugin system