# Volt.js Realtime: Live Data, Effortlessly

**Volt.js Realtime** is the framework's integrated solution for pushing live data from the server to connected clients. Built on top of the robust and simple **Server-Sent Events (SSE)** web standard, it allows you to build real-time features like live UI updates, notifications, and activity feeds with minimal effort and maximum type safety.
...
Under the hood, when you enable realtime features on the client, the `VoltProvider` establishes a single, persistent SSE connection to a dedicated endpoint on your Volt.js server.
...
This is the "magic" of Volt.js Realtime. You can trigger a client-side data refetch directly from your backend mutation, ensuring that any user viewing that data sees the update instantly.
...
export const postController = volt.controller({
  path: '/posts',
  actions: {
...
4.  The `VoltProvider` on the client receives this event.
...
Broadcasting a revalidation event to every single client is not always desirable, especially for user-specific data (e.g., updating a user's own profile). Volt.js allows you to target specific clients by using **scopes**.
...
In your `VoltProvider`, you must define the scopes for the current client using the `getScopes` prop. This function receives the client context and should return an array of scope strings.
...
Define a `query` action and set the `stream` property to `true`. This tells Volt.js that this endpoint is not for fetching data via a single request, but for opening a persistent subscription channel.
...
export const notificationController = volt.controller({
  path: '/notifications',
  actions: {
...
From anywhere in your backend application, you can now publish messages to this stream using the `volt.realtime.publish()` method.

```typescript
// For example, in another action that triggers a notification
someOtherAction: volt.mutation({
  handler: async ({ context, response }) => {
    // ... do some work ...

    // Now, publish a message to the 'notifications.stream' channel.
    // All clients subscribed via `useStream` will receive this data.
    volt.realtime.publish('notifications.stream', {
...
- [VoltProvider](/docs/client-side/volt-provider) - Configure the realtime provider
- [Volt.js Store](/docs/advanced-features/store) - Explore caching and messaging