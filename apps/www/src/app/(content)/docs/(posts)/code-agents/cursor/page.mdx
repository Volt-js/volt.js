# Using Cursor with Volt.js

Cursor is an AI-first code editor designed for pair-programming with a powerful AI. To make Cursor an expert on your Volt.js project, you can configure it with "Rules" that provide persistent context.

## Method 1: Training Cursor on the Volt.js Framework

This method gives Cursor a deep, foundational understanding of the entire Volt.js framework by pointing it to our official, comprehensive documentation file. This is ideal when working on an existing project that wasn't created from one of our templates.

### Step 1: Create a Rules Directory
In the root of your project, create a new directory: `.cursor/rules/`.

### Step 2: Create a Rule File
Inside that directory, create a new file named `volt-framework.mdc`.

### Step 3: Add the Rule Content
Open `volt-framework.mdc` and paste the following content. This rule instructs Cursor's AI to use our `llms.txt` as its primary source of knowledge.
