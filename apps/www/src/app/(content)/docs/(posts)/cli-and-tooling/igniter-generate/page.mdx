# `volt generate`: Scaffolding & Schema Generation

The `volt generate` command is a powerful, multi-purpose tool designed to accelerate your development workflow. It serves three primary functions:
...
npx @volt.js/cli generate feature user --schema prisma:User
```
...
npx @volt.js/cli generate feature dashboard
```
...
The `volt generate schema` command is responsible for the end-to-end type safety of your application. It reads your backend's `AppRouter` and creates a type-safe client that your frontend can use.

### `volt generate schema`
...
npx @volt.js/cli generate schema
```
```json
// Example in package.json
{
  "scripts": {
    "build": "volt generate schema && next build"
  }
}
```

### `volt generate schema --watch`
...
This is used internally by the main `volt dev` command to provide a seamless, real-time development experience. You typically won't need to run this command manually.

---

## 4. OpenAPI Documentation Generation

The `volt generate docs` command creates comprehensive OpenAPI 3.0 documentation from your existing API controllers and routes. This ensures your documentation stays in sync with your actual implementation.
...
npx @volt.js/cli generate docs

# Generate to custom location
npx @volt.js/cli generate docs --output ./docs/api-spec.json
```

### `volt generate docs --serve`

Generate documentation and immediately serve it with a web UI:

```bash
# Generate and serve documentation at http://localhost:3001
npx @volt.js/cli generate docs --serve

# Serve on custom port
npx @volt.js/cli generate docs --serve --port 8080
```
...
For detailed configuration options, advanced customization, and integration examples, see [volt generate docs](/docs/cli-and-tooling/volt-generate-docs).

---

## Next Steps

- [volt generate docs](/docs/cli-and-tooling/volt-generate-docs) - Deep dive into OpenAPI documentation generation
- [volt init](/docs/cli-and-tooling/volt-init) - Learn about project scaffolding
- [volt dev](/docs/cli-and-tooling/volt-dev) - Discover the interactive development server
