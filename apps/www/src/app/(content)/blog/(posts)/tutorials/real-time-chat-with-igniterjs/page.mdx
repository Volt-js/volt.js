# Build a Production-Ready, Real-Time Chat App with Volt.js, Next.js, and Prisma

<Callout type="info" title="What You'll Build">
In this comprehensive, step-by-step guide, you will build a fully type-safe, production-ready, real-time chat application. We'll leverage the power of **Volt.js** and its native Server-Sent Events (SSE) support for instant message delivery. The stack includes **Next.js (App Router)** for the frontend, **Prisma** as our ORM, and **PostgreSQL** for the database.

You'll discover how Volt.js accelerates development with its CLI, automatic code generation, and a beautifully simple set of hooks for data fetching and real-time communication.

**Check out the live demo:** [**volt-js-sample-realtime-chat.vercel.app**](https://volt-js-sample-realtime-chat.vercel.app/)
</Callout>

---

## The Magic of Real-Time: Why Volt.js and SSE?
...
**Volt.js embraces SSE as a first-class citizen, offering:**
...
-   **Automatic UI Revalidation:** Volt.js introduces a powerful `revalidate` mechanism. When a mutation happens on the backend (like sending a message), you can tell the server to notify all clients to refetch their data. This updates the UI automatically, often eliminating the need for manual client-side state management for real-time updates.
-   **AI-Powered Productivity:** With the Volt.js CLI, you can generate entire API features directly from your Prisma schema, creating controllers, validation, and types in seconds.

<Card title="View the Final Source Code on GitHub" href="https://github.com/andeerc/volt-js/tree/main/apps/sample-realtime-chat" />
...
## Step 1: Initialize Your Project with the Volt CLI

The fastest way to start a new Volt.js project is with the official CLI. It scaffolds a new application based on a chosen template, setting up all the necessary boilerplate for you.

Open your terminal and run the `volt init` command:

```bash
npx @volt-js/cli@latest init
```

The CLI will prompt you to choose a template. For this tutorial, select the **Next.js App Router** template. This will create a new directory with a fully configured Next.js project, including Volt.js, TypeScript, and Tailwind CSS.
...
```env
# ./.env
DATABASE_URL="postgresql://user:password@localhost:5432/volt_chat?schema=public"
```
...
This is where the magic of Volt.js's CLI tooling comes into play. Instead of manually writing controllers, validation logic, and type definitions for our `Message` model, we can use the `volt generate` command to do it for us.
...
```bash
npx @volt-js/cli generate feature message --schema prisma:Message
```

<Callout type="success" title="Scaffold Code Generation">
What just happened? The Volt CLI read your `Message` model from `schema.prisma` and generated a complete set of CRUD (Create, Read, Update, Delete) operations for it. This includes:
...
export const messageController = volt.controller({
  ...
  actions: {
    ...
    list: volt.query({
      ...
      path: '/',
      stream: true, // Add this flag tells Volt this is an SSE endpoint
...
Open `src/volt.router.ts` and import the `messageController`. Then, add it to the `controllers` object.
...
If you're using tools like Cursor, Windsurf, GitHub Copilot, Claude Code, or similar, Volt.js comes starter-ready with specific training for Lia. She deeply understands how to use Volt.js: from operating the CLI, creating features, configuring framework processes, creating jobs, writing tests, and much more. This means you can simply ask Lia to test routes, generate cURL commands, create automated tests, or even scaffold entire features — all as part of the framework's AI-friendly workflow.
</Callout>
...
-   **Horizontal Scaling:** If you need to scale your application across multiple server instances, you'll need a shared message bus for real-time events. Volt.js has an adapter for Redis (`@volt-js/adapter-redis`) that handles this seamlessly. When you call `volt.realtime.publish()`, the Redis adapter will publish the message to a Redis channel, and all your server instances subscribed to that channel will receive it and forward it to their connected clients.
-   **Telemetry:** For production monitoring, consider using the `@volt-js/adapter-opentelemetry` to trace requests and real-time flows, helping you diagnose issues and monitor performance.
...
<Accordion>
  <Expandable title="I don't see new messages in real-time">
    <ul>
      ...
      <li>Check that <code>VoltProvider</code> has <code>enableRealtime={true}</code> and the correct <code>baseURL</code> and <code>basePATH</code>.</li>
...
You've just scratched the surface of what's possible with Volt.js. Here are some resources to continue your journey:

-   [**Live Demo**](https://volt-js-sample-realtime-chat.vercel.app/)
...
<Card title="Source Code: Real-Time Chat Sample" href="https://github.com/andeerc/volt-js/tree/main/apps/sample-realtime-chat" />
