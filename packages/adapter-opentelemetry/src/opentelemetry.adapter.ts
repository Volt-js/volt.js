import {
  type VoltTelemetryProvider,
  type VoltTelemetrySpan,
  type VoltTimer,
...
/**
 * OpenTelemetry adapter implementation for Volt.js telemetry system
 */
export class OpenTelemetryAdapterImpl implements OpenTelemetryAdapter {
  ...
  private _activeSpan: VoltTelemetrySpan | null = null;
  private _logger = VoltConsoleLogger.create({
    level: process.env.NODE_ENV === 'production' ? VoltLogLevel.INFO : VoltLogLevel.DEBUG,
    context: {
  ...
  private _createNoOpSpan(name: string): VoltTelemetrySpan {
    return {
      name,
      id: 'noop',
      traceId: 'noop',
