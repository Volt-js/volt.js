...
export function generateDockerCompose(config: ProjectSetupConfig): TemplateFile | null {
  ...
  const hasDatabase = config.database.provider !== 'none'
  const hasRedis = config.features.store
  
  if (!hasDatabase && !hasRedis) return null

  let content = `# Docker Compose for ${config.projectName}
# Generated by @volt.js/cli
