 ...
 */
export const VOLT_FEATURES: Record<string, FeatureDefinition> = {
  store: {
    key: 'store',
    name: 'Redis Store',
    description: 'Caching, sessions, and pub/sub messaging',
    dependencies: [
      { name: '@volt.js/adapter-redis', version: 'latest' },
  ...
  jobs: {
    ...
    dependencies: [
      { name: '@volt.js/adapter-redis', version: 'latest' },
      { name: '@volt.js/adapter-bullmq', version: 'latest' },
    ...
    envVars: [
      ...
      { key: 'VOLT_JOBS_QUEUE_PREFIX', value: 'volt', description: 'Job queue prefix' }
    ]
  },
  ...
  mcp: {
    ...
    dependencies: [
      { name: '@volt.js/adapter-mcp', version: 'latest' },
    ...
    envVars: [
      { key: 'VOLT_MCP_SERVER_BASE_PATH', value: '/api/mcp', description: 'MCP server base path' },
      { key: 'VOLT_MCP_SERVER_TIMEOUT', value: '3600000', description: 'MCP session timeout in ms' },
      { key: 'REDIS_URL', value: 'redis://localhost:6379', description: 'Redis connection URL' },
      { key: 'REDIS_HOST', value: 'localhost', description: 'Redis host' },
      { key: 'REDIS_PORT', value: '6379', description: 'Redis port' }
    ]
  ...
  logging: {
    ...
    dependencies: [
      { name: '@volt.js/core', version: 'latest' }
    ],
    envVars: [
      { key: 'VOLT_LOG_LEVEL', value: 'info', description: 'Logging level (debug, info, warn, error)' },
    ]
  },
  ...
  telemetry: {
    ...
    envVars: [
      { key: 'VOLT_TELEMETRY_ENABLE_TRACING', value: 'true', description: 'Enable telemetry tracing' },
      { key: 'VOLT_TELEMETRY_ENABLE_METRICS', value: 'true', description: 'Enable telemetry metrics' },
      { key: 'VOLT_TELEMETRY_ENABLE_EVENTS', value: 'true', description: 'Enable telemetry metrics' },
      { key: 'VOLT_TELEMETRY_ENABLE_CLI_INTEGRATION', value: 'true', description: 'Enable telemetry metrics' },
    ]
  }
}
