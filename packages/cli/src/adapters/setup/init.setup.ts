...
export async function handleInitCommand(
  ...
  try {
    ...
    logger.info(`Starting Volt.js project initialization (existing project: ${isExistingProject})`, {
      targetDir,
      options,
    });
    ...
    if (isExistingProject) {
      ...
      console.log(chalk.dim("Let's add Volt.js to your existing project..."));
      console.log();
    }
...
async function checkDirectoryAndConfirm(
  ...
  try {
    ...
    if (isExistingProject) {
      const voltCoreFile = path.join(targetDir, 'src', 'volt.ts');
      if (await fs.stat(voltCoreFile).catch(() => null)) {
        console.log(chalk.yellow('⚠️  It looks like Volt.js might already be set up here.'));
...
export function showInitHelp(): void {
  console.log(chalk.bold('\n🔥 Volt.js Init Command\n'))

  console.log(chalk.bold('Usage:'))
  console.log('  volt init [project-name] [options]')
  console.log('  volt init . [options]            # Initialize in current directory')
  console.log()

  console.log(chalk.bold('Examples:'))
  console.log('  volt init my-api                 # Create new project')
  console.log('  volt init .                      # Initialize current directory')
  console.log('  volt init my-api --force         # Skip confirmation prompts')
  console.log()
