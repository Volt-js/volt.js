...
import { startVoltWithFramework } from './concurrent-processes'
...
export interface DevServerOptions {
  ...
  cwd?: string
  /**
   * Use concurrently to run with Volt watcher
   * @default false
   */
  withVolt?: boolean
  /**
   * Custom Volt watcher command
   */
  voltCommand?: string
}
...
export async function startDevServer(options: DevServerOptions = {}): Promise<ChildProcess | void> {
  ...
  // If withVolt option is enabled, use concurrent processes
  if (options.withVolt) {
    logger.info('Starting dev server with Volt watcher', {
      framework,
      packageManager,
      command,
      port,
      voltCommand: options.voltCommand
    });

    try {
      await startVoltWithFramework({
        framework,
        frameworkCommand: command,
        cwd,
        port,
        debug: options.debug,
        voltWatcherCommand: options.voltCommand
      });
...
export async function startVoltDev(options: {
  ...
  const logger = createChildLogger({ component: 'volt-dev' });
  ...
  logger.info('Starting Volt development mode', {
    framework,
    packageManager,
    frameworkCommand,
  ...
  try {
    ...
    logger.error('Failed to start Volt development', { error: formatError(error) });
    throw error;
  }
} 