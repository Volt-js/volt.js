...
class InteractiveProcessManager {
  ...
  private voltTabIndex: number; // Index of the Volt tab
  ...
  constructor(configs: ProcessConfig[]) {
    // Reorganize configs: Framework first, then Volt
    this.processConfigs = this.reorganizeConfigs(configs);

    // Set tab indices based on new order
    this.voltTabIndex = 1; // Volt is always second
  ...
  /**
   * Reorganize configs to put framework first, then Volt
   */
  private reorganizeConfigs(configs: ProcessConfig[]): ProcessConfig[] {
    ...
    const voltConfig = configs.find(c =>
      c.name.toLowerCase().includes('volt')
    );

    const otherConfigs = configs.filter(c => c !== frameworkConfig && c !== voltConfig);

    const result = [];
    if (frameworkConfig) result.push(frameworkConfig);
    if (voltConfig) result.push(voltConfig);
  ...
  private getProjectInfo(): ProjectInfo {
    try {
      ...
      if (fs.existsSync(packageJsonPath)) {
        ...
        // Try to find Volt version from dependencies
        const allDeps = {
        ...
        return {
          ...
          voltVersion: voltVersion ? voltVersion.replace(/[\^~]/, '') : undefined
        };
      }
    } catch (error) {
  ...
  private async testRedisConnection() {
    try {
      ...
      await subscriber.subscribe('volt:api-requests', 'volt:telemetry');

      subscriber.on('message', (channel, message) => {
        if (channel === 'volt:api-requests') {
          try {
        ...
        } else if (channel === 'volt:telemetry') {
          try {
            const data = JSON.parse(message);
  ...
  private drawHeader() {
    ...
    const versionInfo = this.projectInfo.voltVersion ? `v${this.projectInfo.voltVersion}` : '';
    const projectName = this.projectInfo.name !== 'Unknown Project' ? ` - ${this.projectInfo.name}` : '';

    const title = `${ANSI_COLORS.bold}${ANSI_COLORS.blue}Volt.js ${versionInfo}${projectName}${ANSI_COLORS.reset}`;
  ...
  private drawTabs() {
    ...
    tabs.push(`${apiStyle}${apiColor}${apiIcon} ${this.apiTabIndex + 1}. Volt.js API${ANSI_COLORS.reset}`);
    ...
    tabs.push(`${jobsStyle}${jobsColor}${jobsIcon} ${this.jobsTabIndex + 1}. Volt.js Jobs${queueDisplay}${ANSI_COLORS.reset}`);
  ...
  private drawApiRequests() {
    ...
    if (visibleRequests.length === 0) {
      console.log(`${ANSI_COLORS.dim}No API requests yet. See docs at https://volt.js.org/docs/api${ANSI_COLORS.reset}\n`);
      return;
    }
  ...
  private drawJobsContent() {
    ...
    if (visibleJobs.length === 0) {
      console.log(`${ANSI_COLORS.dim}No jobs found. See docs at volt.js.org/docs/jobs${ANSI_COLORS.reset}\n`);
      return;
    }
...
/**
 * Create a process config for the Volt watcher
 */
export function createVoltWatcherProcess(options: {
  cwd?: string;
  debug?: boolean;
}): ProcessConfig {
  return {
    name: 'Volt',
    command: `volt watch${options.debug ? ' --debug' : ''}`,
    color: PROCESS_COLORS.volt,
    cwd: options.cwd,
    env: {
...
/**
 * Start both Volt watcher and framework dev server concurrently
 */
export async function startVoltWithFramework(options: {
  framework: SupportedFramework;
  frameworkCommand: string;
  cwd?: string;
  port?: number;
  debug?: boolean;
  voltWatcherCommand?: string;
}): Promise<void> {
  const logger = createChildLogger({ component: 'volt-with-framework' });

  logger.info('Starting Volt with framework', {
  ...
  // Override volt process command if provided
  if (options.voltWatcherCommand) {
    processes[0].command = options.voltWatcherCommand;
  }
