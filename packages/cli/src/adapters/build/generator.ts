...
import type { VoltBuildConfig } from './watcher'
import { createChildLogger } from '../logger'
import { createDetachedSpinner } from '@/lib/spinner'

type VoltRouterSchema = {
  config: {
    baseURL: string
    basePATH: string
  }
...
/**
 * Generate client files from Volt router
 */
export async function generateSchemaFromRouter(
  router: VoltRouter,
  config: VoltBuildConfig
) {
  ...
  try {
    ...
    if (!isInteractiveMode) {
      const files = [
        { name: 'volt.schema.ts', path: filePaths[0] },
        { name: 'volt.client.ts', path: filePaths[1] },
      ]
      ...
      logger.separator()
      logger.success('Volt.js development server is up and running')
...
async function generateSchemaFile(
  ...
  const content = `// Generated by @volt-js/cli - DO NOT EDIT
...
`
  ...
  const filePath = path.join(outputDir, 'volt.schema.ts')
...
async function generateClientFile(
  ...
  const filePath = path.join(outputDir, 'volt.client.ts')
...
baseURL: process.env.NEXT_PUBLIC_VOLT_API_URL, // Adapt for your needs
basePath: process.env.NEXT_PUBLIC_VOLT_API_BASE_PATH,
router: () => {
 ...
 */
function generateFileHeader(config: VoltBuildConfig): string {
 ...
 *
 * This file was automatically generated from your Volt router.
