...
/**
 * File watcher for Volt controllers
 * Monitors controller files and regenerates client schema on changes
 */
export class VoltWatcher {
  private watcher: any = null
  private config: VoltBuildConfig
  ...
  constructor(config: VoltBuildConfig) {
    this.config = {
      extractTypes: true,
  ...
  private async regenerateSchema() {
    ...
    try {
      ...
      const possibleRouterPaths = [
        'src/volt.router.ts', 'src/volt.router.js',
        'src/router.ts', 'src/router.js',
        'volt.router.ts', 'volt.router.js',
        'router.ts', 'router.js'
      ]
      ...
      if (error instanceof RouterLoadError) {
        console.error(chalk.red.bold('\n✗ Error loading your Volt router:'));
