...
program
  .name("volt")
  .description("CLI for Volt.js type-safe client generation")
  .version("1.0.0")
...
program
  .command("init")
  .description("Create a new Volt.js project with interactive setup")
...
program
  .command("dev")
  .description("Start development mode with framework and Volt (interactive dashboard and OpenAPI docs by default)")
  ...
  .option("--no-framework", "Disable framework dev server (Volt only)")
  ...
  .action(async (options) => {
    ...
    processes.push({
      name: "Volt",
      command: `volt generate schema --watch --framework ${framework} --output ${options.output}${docsFlags}`,
      color: "blue",
      cwd: process.cwd()
    });
    if (useInteractive) {
...
generate
  .command("schema")
  .description("Generate client schema from your Volt router (for CI/CD or manual builds)")
  .option("--framework <type>", `Framework type (${getFrameworkList()}, generic)`)
  ...
  .action(async (options) => {
    ...
    const { VoltWatcher } = await import("./adapters/build/watcher");
    const watcher = new VoltWatcher({
      framework,
...
generate
  ...
  .action(async (options) => {
    ...
    try {
      ...
      let router: VoltRouter<any, any, any, any, any> | null = null;
      ...
      if (!router) {
        docsLogger.error('No Volt router found in your project. Please ensure you have a router file (e.g., src/volt.router.ts).');
        process.exit(1);
      }
