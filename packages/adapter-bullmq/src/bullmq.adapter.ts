 ...
 * 
 * This adapter provides a unified interface for Volt to interact with BullMQ,
 ...
 * @returns A complete `VoltJobQueueAdapter` implementation
 * 
 * @example
 * ```typescript
 ...
 * const jobQueue = createBullMQAdapter<VoltAppContext>({ store: redisStore });
 * 
 * const volt = Volt
 ...
 * ```
 */
export function createBullMQAdapter<TContext extends object>(options: BullMQAdapterOptions = {}): VoltJobQueueAdapter<TContext> {
  if (!isServer) {
    return {} as VoltJobQueueAdapter<TContext>;
  }
    ...
    try {
      const response = await fetch(webhookUrl, {
        ...
        headers: {
          ...
          'User-Agent': 'Volt-Jobs-Webhook/1.0',
        },
        body: JSON.stringify({
          ...payload,
