...
/**
 * Creates an MCP (Model Context Protocol) adapter for an Volt router.
 * This adapter transforms Volt actions into MCP tools that can be used by AI models.
 * 
 ...
 * 
 * @param router - The Volt router to adapt
...
function resolveInstructionsSync<TContext extends object, TInferredContext>(
  ...
  if (!options.instructions) {
    return `You have access to ${tools.length} tools from the Volt router: ${tools.map(t => t.name).join(', ')}`;
  }
