import type { VoltRouter } from "../types";
...
const PACKAGE_CONFIG = {
  server: [
    // Apenas adapters do Volt que são definitivamente server-only
    ...VOLT_PACKAGES,
  ],
  client: [
    // Apenas Node.js built-ins que sempre causam problema
    ...NODE_MODULES,
    // E alguns adapters específicos do Volt
    ...VOLT_PACKAGES,
  ],
} as const;
 ...
 */
const safeLog = (() => {
  let logger: ReturnType<typeof VoltConsoleLogger.create> | null = null;

  const getLogger = () => {
    if (!logger) {
      try {
        logger = VoltConsoleLogger.create({
...
const createEmptyModule = () => {
  ...
  const showWarning = (prop) => {
    if (!warningShown.has(prop) && typeof console !== 'undefined' && console.warn) {
      console.warn(\`[Volt] Attempted to use server-only module "\${prop}" in client bundle. This is likely a bug.\`);
...
/**
 * Adapter function to convert an VoltRouter instance into Next.js route handlers
 *
 * @param router - An instance of VoltRouter that will handle the incoming requests
 * @returns An object containing HTTP method handlers compatible with Next.js route handlers
 * @example
 * ```typescript
 * const router = new VoltRouter()
 ...
 */
export const nextRouteHandlerAdapter = (router: VoltRouter<any, any, any, any, any>) => {
  return {
    GET: (request: Request) => {
...
export const withVolt = (...configs: NextConfig[]): NextConfig => {
  try {
