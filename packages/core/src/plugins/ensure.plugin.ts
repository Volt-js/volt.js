/**
 * @fileoverview Ensure - Type-safe validation and assertion utility for Volt.js
 * 
 ...
 * ```typescript
 * const ensurePlugin = createVoltPlugin({
 *   name: 'ensure',
...
import { createVoltPlugin, createVoltPluginAction, type VoltPlugin } from "../types/plugin.interface"
import { z } from "zod"
...
export const ensure = createVoltPlugin({
  name: 'ensure',
  $meta: {
    name: 'ensure',
  ...
  registration: {
    ...
    category: ['core'],
    author: 'Volt',
    repository: 'https://github.com/volt-framework/volt',
    documentation: 'https://github.com/volt-framework/volt',
  },
  $actions: {
    ensure: createVoltPluginAction({
      name: 'ensure',
      description: 'Ensure service',
      input: z.object({
        value: z.string(),
