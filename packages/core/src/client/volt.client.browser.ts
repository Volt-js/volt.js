...
import type { VoltAction, VoltControllerConfig, VoltRouter, InferRouterCaller, ClientConfig } from '../types';
import { parseURL } from '../utils/url';
import { createUseQuery, createUseMutation, createUseRealtime } from './volt.hooks';

/**
 * Creates a browser-side client for Volt Router
 ...
 */
export function createBrowserClient<TRouter extends VoltRouter<any, any, any, any, any>>(
  router: TRouter
): InferRouterCaller<TRouter> {
  ...
  for (const controllerName in router.controllers) {
    ...
    const controller = router.controllers[controllerName] as VoltControllerConfig<any>;
    const parsedBaseURL = parseURL(baseURL, basePATH, controller.path);

    for (const actionName in controller.actions) {
      const action = controller.actions[actionName] as VoltAction<any, any, any, any, any, any, any, any, any, any>;
 ...
 */
function createActionFetcher<TAction extends VoltAction<any, any, any, any, any, any, any, any, any, any>>(
  action: TAction,
  baseURL: string,
) {
  return async (options?: TAction['$Infer']['$Input']): Promise<TAction['$Infer']['$Response']> => {
    ...
    try {
      ...
      if (error instanceof Error) {
        ...
        throw new Error(`VoltClient fetch error: ${error.message}`);
      }
      throw error;
    }
  };
