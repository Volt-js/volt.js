...
import { useVoltQueryClient } from "./volt.context";
...
type InferVoltResponse<T> = T extends { data: infer TData, error: infer TError } ? { data: TData | null, error: TError | null } : { data: null, error: null };
...
>() => {
  return () => {
    const { register, unregister, invalidate } = useVoltQueryClient();
...
export const createUseQuery = <
  TAction extends VoltAction<any, any, any, any, any, any, any, any, any, any>,
>(
  controller: string,
  action: string,
    ...
    const retry = useCallback(() => {
      if (lastUsedParamsRef.current) {
        ...
        const logger = VoltConsoleLogger.create({
          ...
          context: createLoggerContext('VoltHooks')
        });
...
export function useRealtime<T = any>(
  ...
  const { subscribeToRealtime } = useVoltQueryClient();
 ...
 */
export const createUseRealtime = <TAction extends VoltAction<any, any, any, any, any, any, any, any, any, any>>(
  controller: string,
  action: string,
) => {
  return (options?: RealtimeActionCallerOptions<TAction>): RealtimeActionCallerResult<TAction> => {
    ...
    // @ts-expect-error - Ignore type error for now
    const [response, setResponse] = useState<InferVoltResponse<Awaited<TAction["$Infer"]["$Output"]>>>(() => ({
      data: options?.initialData?.data || null,
      error: options?.initialData?.error || null,
    }));
