...
import { VoltConsoleLogger } from "../services";
import { VoltLogLevel, type VoltRouter } from "../types";
import type {
  VoltContextType,
  RefetchFn,
...
/**
 * Volt context provider type
 */
const VoltContext = createContext<VoltContextType<any> | undefined>(
  undefined,
);

/**
 * Options for the Volt Provider
 */
export interface VoltProviderOptions<TContext extends () => Promise<any> | any> {
  /**
  ...
  /**
   * Get the context for the VoltProvider
   */
  getContext?: TContext;

  /**
   * The context to be passed to the VoltProvider
   */
  getScopes?: (ctx: Awaited<ReturnType<TContext>>) => Promise<string[]> | string[];
}

/**
 * Provider component for the Volt context, managing query invalidation and refetching.
 *
 * @component
 * @param {PropsWithChildren} props - The component props
 * @param {React.ReactNode} props.children - Child components to be wrapped by the provider
 * @param {VoltProviderOptions} props.options - Configuration options
 *
 * @example
 * ```tsx
 * <VoltProvider options={{ enableRevalidation: true }}>
 *   <App />
 * </VoltProvider>
 * ```
 *
 * @remarks
 * The VoltProvider manages a collection of query listeners and provides methods to:
 ...
 *
 * @returns {JSX.Element} Provider component wrapping its children with Volt context
 */
export function VoltProvider<TContext extends () => Promise<any> | any>({
  children,
  getContext,
  getScopes,
  debug,
  ...
  reconnectDelay = 1000,
}: PropsWithChildren & VoltProviderOptions<TContext>) {
  ...
  const logger = useMemo(() => {
    return VoltConsoleLogger.create({
      level: debug ? VoltLogLevel.DEBUG : VoltLogLevel.INFO,
      context: {
        provider: 'VoltProvider',
        package: 'core'
      }
    });
  }, []);
  ...
  return (
    <VoltContext.Provider value={contextValue}>
      {children}
...
/**
 * Hook to access the Volt context, providing access to query registration and invalidation methods.
 *
 * @returns {VoltContextType} Volt context object
 *
 * @throws {VoltError} Throws an error if the hook is used outside of an VoltProvider
 */
export const useVoltQueryClient = <
  TRouter extends VoltRouter<any, any, any, any, any>,
>() => {
  const context = useContext(VoltContext) as
    | VoltContextType<TRouter>
    | undefined;

  if (!context) {
    throw new Error(
      "useVoltQueryClient must be used within an VoltProvider",
    );
  }
