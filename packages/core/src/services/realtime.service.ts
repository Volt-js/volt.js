...
/**
 * Type-safe, fluent RealtimeBuilder implementation for Volt.js.
 *
...
class RealtimeBuilderImpl<TContext = unknown> implements RealtimeBuilder<TContext> {
   ...
   */
  constructor(store: VoltStoreAdapter, initial: RealtimeEventPayload) {
    this.store = store;
...
/**
 * VoltRealtimeService provides a type-safe, developer-friendly API for realtime event publishing.
 *
 * @typeParam TContext - The context type available to event scopes.
 *
 * @example
 * // Create the service (usually injected via builder)
 * const realtime = new VoltRealtimeService(store);
 *
 ...
 */
export class VoltRealtimeService<TContext = any>
  implements VoltRealtimeServiceType<TContext> {
  private readonly store: VoltStoreAdapter;

  /**
   * Construct a new VoltRealtimeService.
   *
   * @param store - The store adapter for event publishing (reserved for future use).
   *
   * @example
   * const realtime = new VoltRealtimeService(store);
   */
  constructor(store: VoltStoreAdapter) {
    this.store = store;
  }
