...
import type { ContextCallback, VoltBaseContext } from "../types/context.interface";
...
/**
 * 100% Type-Safe Plugin Manager for Volt.js with StoreAdapter Integration
 *
 ...
 * @example
 * ```typescript
 * const pluginManager = new VoltPluginManager<MyContext>({
 ...
 * ```
 */
export class VoltPluginManager<
...
> {
  ...
  private readonly store: VoltStoreAdapter;
  private readonly logger: VoltLogger;
   ...
   *
   * Events are published to Redis channels with pattern: `volt:plugin:events:{eventName}`
  ...
  async emit<TPayload>(
    ...
    try {
      ...
      const eventChannel = `volt:plugin:events:${eventName}`;
  ...
  private createPluginProxy<TActions extends PluginActionsCollection<any>>(
    plugin: VoltPlugin<any, any, any, any, any, any, any, any>,
  ): void {
