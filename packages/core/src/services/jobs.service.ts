 ...
 */
export interface VoltJobsServiceConfig<TContext extends object> {
  /** The job queue adapter to use (e.g., BullMQ) */
  adapter: VoltJobQueueAdapter<TContext>;
 ...
 * 
 * This service acts as the primary interface for job management in Volt applications,
...
> {
  ...
  bulkRegister<TNewJobs extends Record<string, JobDefinition<TContext, any, any>>>(
    jobs: TNewJobs
  ): VoltJobsService<TContext, TJobs & TNewJobs> {
    ...
    const newService = new VoltJobsService<TContext, TJobs & TNewJobs>({
 ...
 * ```
 */
export function createVoltJobsService<TContext extends object>(
  config: VoltJobsServiceConfig<TContext>
): VoltJobsService<TContext> {
  return new VoltJobsService(config);
}
