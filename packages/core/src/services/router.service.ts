import type { ContextCallback, DocsConfig, VoltBaseConfig, VoltControllerConfig, VoltRouter } from "../types";
...
/**
 * Creates a fully-typed, production-ready router instance for the Volt Framework.
 *
 ...
 * @template TConfig extends VoltBaseConfig - The router configuration object
 * @template TPlugins extends Record<string, VoltPlugin<any, any, any, any, any, any, any, any>> - Record of plugins available in this router
 *
 ...
 *
 * @returns A fully-configured, type-safe VoltRouter instance:
 ...
 * ---
 * ## See Also
 * - [Controller Documentation](https://voltjs.dev/docs/controllers)
 * - [Plugin System](https://voltjs.dev/docs/plugins)
 * - [Context Patterns](https://voltjs.dev/docs/context)
 * - [Testing Guide](https://voltjs.dev/docs/testing)
 */
export const createVoltRouter = <
...
>(params: {
  ...
  plugins?: TPlugins;
  docs?: TDocs;
}): VoltRouter<TContext, TControllers, TConfig, TPlugins, TDocs> => {
  type TRouter = VoltRouter<TContext, TControllers, TConfig, TPlugins, TDocs>;
  ...
  return {
    ...
    handler: async (request: Request) => {
      ...
      if (path.startsWith(parseURL(basePath, playgroundPath))) {
        ...
        const playground = initializeVoltPlayground(params.docs, basePath);
        try {
