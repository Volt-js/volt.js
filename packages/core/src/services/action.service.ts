...
import type { StandardSchemaV1, VoltProcedure, VoltActionHandler, VoltActionContext, QueryMethod, InferEndpoint, VoltQueryOptions, VoltAction, MutationMethod } from "../types";

/**
 * Creates a type-safe query action for the Volt Framework.
 ...
 * @example
 * ```typescript
 * const getUsers = createVoltQuery({
 *   path: 'users',
...
/**
 * Creates a type-safe mutation action for the Volt Framework.
 ...
 * @example
 * ```typescript
 * const createUser = createVoltMutation({
 *   path: 'users',
 *   method: 'POST',
 *   body: z.object({
 *     name: z.string(),
 ...
 * ```
 */
export const createVoltMutation = <
  ...
  TMutationMiddlewares extends readonly VoltProcedure<TMutationContext, any, any>[],
  ...
  TMutationHandler extends VoltActionHandler<
    VoltActionContext<TMutationContext, TMutationPath, TMutationMethod, TMutationBody, TMutationQuery, TMutationMiddlewares, TMutationPlugins>,
    any
  >,
