...
/**
 * Creates a reusable middleware procedure for the Volt Framework.
 ...
 * const authProcedure = createVoltProcedure({
 *   name: 'auth',
 ...
 * const protectedAction = createVoltQuery({
 *   path: 'protected',
 *   use: [authProcedure()],
 *   handler: (ctx) => {
 ...
 */
export const createVoltProcedure = <
  TActionContext = unknown,
  TOptions = unknown,
  TOutput = unknown
>(Procedure: VoltProcedure<TActionContext, TOptions, TOutput>) => (options?: TOptions): VoltProcedure<TActionContext, TOptions, TOutput> => {
  return {
    ...Procedure,
    // @ts-expect-error - This is a hack to get around the circular dependency
    handler: (ctx: VoltProcedureContext<TActionContext>) => Procedure.handler(options, ctx)
  }
};
 ...
 * This is the recommended way to create procedures in modern Volt applications.
 * 
 ...
 * const timestampProcedure = volt.procedure()
 *   .name('timestamp')
    ...
    name<TName extends string>(name: TName): EnhancedProcedureBuilderWithName<TActionContext, TName> {
      return {
        options<TOptionsSchema extends StandardSchemaV1>(
          ...
          return {
            ...
            > & { name: TName } {
              return {
                name,
                handler: (options: StandardSchemaV1.InferInput<TOptionsSchema>, ctx: VoltProcedureContext<TActionContext>) => {
              ...
              } as VoltProcedure<TActionContext, StandardSchemaV1.InferInput<TOptionsSchema>, TOutput> & { name: TName };
            }
          };
        },
        ...
        handler<TOutput>(
          ...
          return {
            name,
            handler: (_options: undefined, ctx: VoltProcedureContext<TActionContext>) => {
          ...
          } as VoltProcedure<TActionContext, undefined, TOutput> & { name: TName };
        }
      };
    },
      ...
      return {
        ...
        handler: (
          options: TOptionsSchema extends StandardSchemaV1
            ? StandardSchemaV1.InferInput<TOptionsSchema>
            : undefined,
          ctx: VoltProcedureContext<TActionContext>
        ) => {
          // Validate options using the schema
          const validatedOptions = (config.optionsSchema as any).parse ? (config.optionsSchema as any).parse(options) : options;
