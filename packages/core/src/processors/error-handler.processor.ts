...
/**
 * Error handler processor for the Volt Framework.
...
export class ErrorHandlerProcessor {
  ...
  /**
   * Handles VoltError instances.
   *
   * @param error - The VoltError instance
   * @param context - The processed context
   * @param telemetrySpan - The telemetry span for tracking
   * @param startTime - Request start time
   * @returns Standardized error response
   */
  static async handleVoltError(
    error: VoltError,
    context: ProcessedContext,
    telemetrySpan: TelemetrySpan | null,
    startTime: number
    ...
    this.logger.error("Volt framework error", {
      error: {
        code: normalizedError.code,
        message: normalizedError.message,
        details: normalizedError.details
    ...
    // Track volt error for CLI dashboard
    await this.trackError(context, startTime, statusCode, error);
  ...
  static async handleError(
    ...
    // VoltError instances
    if (error instanceof VoltError) {
      return this.handleVoltError(error, context, telemetrySpan, startTime);
    }
  ...
  private static async trackError(
    ...
    try {
      ...
      const fallbackLogger = VoltConsoleLogger.create({
