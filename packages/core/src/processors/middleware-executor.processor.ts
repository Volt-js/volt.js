...
import { VoltLogLevel, type VoltLogger, type VoltProcedure } from "../types";
import type { ProcessedContext } from "./context-builder.processor";
import { VoltConsoleLogger } from "../services/logger.service";
import type { VoltProcedureContext } from "../types/procedure.interface";
...
/**
 * Middleware executor processor for the Volt Framework.
...
export class MiddlewareExecutorProcessor {
  ...
  static async executeGlobal(
    ...
    middlewares: VoltProcedure<any, any, any>[]
    ...
    for (let i = 0; i < middlewares.length; i++) {
      ...
      try {
        ...
        if (result instanceof VoltResponseProcessor) {
          this.logger.debug(
            "Middleware early return",
   ...
   * Maps ProcessedRequest to the format expected by VoltProcedureContext.
   *
   * @param context - The processed context
  ...
  private static buildProcedureContext(
    ...
    // Map ProcessedRequest to VoltProcedureContext.request structure
    ...
    if (!procedureRequest.cookies) {
      ...
      procedureRequest.cookies = new VoltCookie(procedureRequest.headers);
    }
    ...
    const procedureContext: VoltProcedureContext<any> = {
      request: procedureRequest,
      context: context.$context || {}, // Use $context, not the full context
      response: context.response || new VoltResponseProcessor(),
      next
    };
