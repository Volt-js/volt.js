...
import type { VoltProcedure } from '../../types/procedure.interface';
...
// Mock VoltCookie
const createMockCookie = (): VoltCookie => {
  const mockHeaders = new Headers();
  const cookie = new VoltCookie(mockHeaders);
...
const createMockContext = (overrides: Partial<ProcessedContext> = {}): ProcessedContext => ({
  ...
  response: new VoltResponseProcessor(),
  $context: {
...
const createMockProcedure = (
  name: string,
  handler: VoltProcedure<any, any, any>['handler'],
  options: Partial<VoltProcedure<any, any, any>> = {}
): VoltProcedure<any, any, any> => ({
  name,
  handler,
  ...options,
});
...
describe('MiddlewareExecutorProcessor', () => {
  describe('buildProcedureContext', () => {
    test('should build correct procedure context from ProcessedContext', () => {
      ...
      expect(procedureContext).toEqual({
        request: {
          ...
          cookies: expect.any(VoltCookie),
        },
        context: context.$context,
        response: expect.any(VoltResponseProcessor),
      });
    });
    ...
    test('should create fallback cookies when missing', () => {
      ...
      expect(procedureContext.request.cookies).toBeInstanceOf(VoltCookie);
    });
  ...
  describe('executeGlobal', () => {
    ...
    test('should handle early return with ResponseProcessor', async () => {
      const context = createMockContext();
      const responseProcessor = new VoltResponseProcessor().unauthorized('Access denied');
