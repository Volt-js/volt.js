...
describe('Response Processor', () => {
  let mockStore: VoltStoreAdapter;
  ...
  describe('Processor Creation and Configuration', () => {
    it('should create processor with static init method', () => {
      const processor = VoltResponseProcessor.init();

      expect(processor).toBeInstanceOf(VoltResponseProcessor);
    });

    it('should create processor with store adapter', () => {
      const processor = VoltResponseProcessor.init(mockStore);

      expect(processor).toBeInstanceOf(VoltResponseProcessor);
    });

    it('should create processor with context and store', () => {
      const processor = VoltResponseProcessor.init<TestContext>(mockStore, testContext);
  ...
  describe('Success Responses', () => {
    it('should create success response with data', async () => {
      ...
      const response = await VoltResponseProcessor.init()
        .success(testData)
        .toResponse();
  ...
  describe('Streaming Responses', () => {
    ...
    it('should handle existing channels', async () => {
      ...
      await VoltResponseProcessor.init()
        .stream({
  ...
  describe('Cache Revalidation', () => {
    it('should set revalidation with query keys array', async () => {
      const response = await VoltResponseProcessor.init<TestContext>(mockStore, testContext)
    ...
    it('should set revalidation with single query key', async () => {
      await VoltResponseProcessor.init<TestContext>(mockStore, testContext)
  ...
  describe('Data Serialization and Types', () => {
    it('should handle primitive data types', async () => {
      const responses = await Promise.all([
        VoltResponseProcessor.init().success('string')
          .toResponse(),
        VoltResponseProcessor.init().success(42)
          .toResponse(),
        VoltResponseProcessor.init().success(true)
          .toResponse(),
        VoltResponseProcessor.init().success(null)
          .toResponse(),
      ]);
  ...
  describe('Response Structure Consistency', () => {
    it('should always return consistent response structure for success', async () => {
      const responses = await Promise.all([
        VoltResponseProcessor.init().success({ test: 1 })
          .toResponse(),
        VoltResponseProcessor.init().created({ test: 2 })
          .toResponse(),
        VoltResponseProcessor.init().json({ test: 3 })
          .toResponse(),
        VoltResponseProcessor.init().noContent()
          .toResponse(),
      ]);
    ...
    it('should always return consistent response structure for errors', async () => {
      const responses = await Promise.all([
        VoltResponseProcessor.init().badRequest('Bad')
          .toResponse(),
        VoltResponseProcessor.init().unauthorized('Unauth')
          .toResponse(),
        VoltResponseProcessor.init().forbidden('Forbidden')
          .toResponse(),
        VoltResponseProcessor.init().notFound('NotFound')
          .toResponse(),
      ]);
    ...
    it('should maintain consistent headers', async () => {
      const responses = await Promise.all([
        VoltResponseProcessor.init().success({})
          .toResponse(),
        VoltResponseProcessor.init().badRequest()
          .toResponse(),
      ]);
