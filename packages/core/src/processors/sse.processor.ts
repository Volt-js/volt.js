import { VoltLogLevel, type VoltLogger } from "../types";
import { VoltError } from "../error";
...
/**
 * Interface for SSE connection storage backend
 */
export interface SSEAdapter {
  addConnection(channelId: string, connectionId: string, metadata?: any): Promise<void>;
  removeConnection(channelId: string, connectionId: string): Promise<void>;
  publishEvent(channelId: string, event: SSEEvent): Promise<number>;
  getConnections(channelId: string): Promise<string[]>;
  cleanup(): Promise<number>;
}

/**
 * Redis-based SSE adapter for distributed SSE connections
 */
export class SSERedisAdapter implements SSEAdapter {
  private redis: Redis;
  private subscriberRedis: Redis;
  private logger: VoltLogger;
  private eventHandlers = new Map<string, (event: SSEEvent) => void>();
  ...
  constructor(redis: Redis, logger?: VoltLogger) {
    this.redis = redis;
    this.subscriberRedis = redis.duplicate();
    this.logger = logger || VoltConsoleLogger.create({
      level: resolveLogLevel(),
      context: createLoggerContext('SSERedisAdapter'),
      showTimestamp: true,
    });
  ...
  private setupSubscriber(): void {
    this.subscriberRedis.on('message', (channel: string, message: string) => {
      try {
        ...
        this.logger.error('Failed to parse SSE event from Redis', { channel, error });
      }
    });
  }
  ...
  private startCleanupInterval(): void {
    setInterval(async () => {
      try {
        ...
        this.logger.error('SSE cleanup failed', { error });
      }
    }, this.CLEANUP_INTERVAL * 1000);
  }

  async addConnection(channelId: string, connectionId: string, metadata?: any): Promise<void> {
    const key = `sse:connections:${channelId}`;
    ...
    await this.redis.pipeline()
      .sadd(key, connectionId)
      .hset(`sse:connection:${connectionId}`, connectionData)
      .expire(`sse:connection:${connectionId}`, this.CONNECTION_TTL)
      .expire(key, this.CONNECTION_TTL)
      .exec();

    // Set timeout for connection cleanup
    const timeout = setTimeout(async () => {
      await this.removeConnection(channelId, connectionId);
    }, this.CONNECTION_TTL * 1000);

    this.connectionTimeouts.set(connectionId, timeout);

    this.logger.debug('SSE connection added to Redis', { channelId, connectionId });
  }

  async removeConnection(channelId: string, connectionId: string): Promise<void> {
    const key = `sse:connections:${channelId}`;

    await this.redis.pipeline()
      .srem(key, connectionId)
      .del(`sse:connection:${connectionId}`)
      .exec();
    ...
    this.logger.debug('SSE connection removed from Redis', { channelId, connectionId });
  }

  async publishEvent(channelId: string, event: SSEEvent): Promise<number> {
    const message = JSON.stringify(event);
    const subscribersCount = await this.redis.publish(`sse:channel:${channelId}`, message);
  ...
  async cleanup(): Promise<number> {
    ...
    for (const key of keys) {
      ...
      if (ttl <= 0) {
        // Connection expired, remove from all channel sets
        const connectionId = key.replace('sse:connection:', '');

        // Find all channels this connection was part of
        const channelPattern = 'sse:connections:*';
    ...
    if (cleanedCount > 0) {
      this.logger.debug('SSE connections cleaned up', { cleanedCount });
    }

    return cleanedCount;
  }

  async subscribeToChannel(channelId: string, handler: (event: SSEEvent) => void): Promise<void> {
    const redisChannel = `sse:channel:${channelId}`;
    this.eventHandlers.set(redisChannel, handler);
    await this.subscriberRedis.subscribe(redisChannel);

    this.logger.debug('Subscribed to SSE channel', { channelId });
  }

  async unsubscribeFromChannel(channelId: string): Promise<void> {
    const redisChannel = `sse:channel:${channelId}`;
    this.eventHandlers.delete(redisChannel);
    await this.subscriberRedis.unsubscribe(redisChannel);

    this.logger.debug('Unsubscribed from SSE channel', { channelId });
  }
  ...
  async disconnect(): Promise<void> {
    ...
    this.logger.debug('SSE Redis adapter disconnected');
  }
}
...
export class SSEProcessor {
  ...
  private static _adapter: SSEAdapter | null = null;
  ...
  /**
   * Set the SSE adapter (Redis or in-memory)
   */
  static setAdapter(adapter: SSEAdapter | null): void {
    this._adapter = adapter;
    this.logger.debug('SSE adapter configured', { type: adapter ? 'Redis' : 'In-Memory' });
  }

  /**
   * Get the current SSE adapter
   */
  static getAdapter(): SSEAdapter | null {
  ...
  /**
   * Map of active connections per channel (fallback for in-memory mode)
   * @private
   */
  ...
  /**
   * Map of connection handlers by connection ID (for Redis mode)
   * @private
   */
  private static connectionHandlers: Map<string, SSEConnectionHandler> = new Map();

  /**
   * Register a new channel for SSE events
   *
   * @param channel - Channel configuration
   * @throws {VoltError} When channel already exists
   */
   ...
   */
  static async getConnectionCount(channelId: string): Promise<number> {
    if (this._adapter) {
      const connections = await this._adapter.getConnections(channelId);
   ...
   */
  static async getTotalConnectionCount(): Promise<number> {
    if (this._adapter) {
      let total = 0;
      for (const channel of this.channels.keys()) {
        const connections = await this._adapter.getConnections(channel);
   ...
   * @throws {VoltError} When channel validation fails
   */
  ...
  private static createSSEStream(options: SSEStreamOptions): Response {
    ...
    const stream = new ReadableStream({
      start: (controller) => {
        ...
        const handlerWithMetadata = {
          handler: connectionHandler,
          scopes,
          metadata: { connectedAt: Date.now() }
        };

        // Store handler reference for Redis mode
        this.connectionHandlers.set(connectionId, handlerWithMetadata);

        for (const channel of channels) {
          if (this._adapter) {
            // Redis mode: register connection in Redis and subscribe to channel
            this._adapter.addConnection(channel, connectionId, {
              scopes,
              connectedAt: Date.now()
            }).catch(error => {
              this.logger.error('Failed to register connection in Redis', { error, channel, connectionId });
            });

            // Subscribe to Redis channel if not already subscribed
            if (this._adapter instanceof SSERedisAdapter) {
              this._adapter.subscribeToChannel(channel, (event: SSEEvent) => {
                // Relay event to local connection handlers
                const handler = this.connectionHandlers.get(connectionId);
                if (handler) {
                  handler.handler(event);
                }
              }).catch(error => {
                this.logger.error('Failed to subscribe to Redis channel', { error, channel });
              });
            }
          } else {
            // In-memory mode: use local connections map
            if (this.connections.has(channel)) {
              const channelConnections = this.connections.get(channel)!;
              channelConnections.add(handlerWithMetadata);
              this.logger.debug(
                "Client subscribed",
        ...
        return () => {
          ...
          // Remove connection handler reference
          this.connectionHandlers.delete(connectionId);

          // Unregister this connection handler from all channels
          for (const channel of channels) {
            if (this._adapter) {
              // Redis mode: remove connection from Redis
              this._adapter.removeConnection(channel, connectionId).catch(error => {
                this.logger.error('Failed to remove connection from Redis', { error, channel, connectionId });
              });
            } else {
              // In-memory mode: remove from local connections map
              if (this.connections.has(channel)) {
                const channelConnections = this.connections.get(channel)!;
                channelConnections.delete(handlerWithMetadata);
                this.logger.debug(
   ...
   */
  static async publishEvent(event: SSEEvent): Promise<number> {
    const { channel } = event;
    ...
    // Use Redis adapter if available, otherwise fallback to in-memory
    if (this._adapter) {
      return await this._adapter.publishEvent(channel, event);
    }
  ...
  static async broadcastEvent(
    ...
    for (const channel of channels) {
      totalSent += await this.publishEvent({
        ...event,
        channel,
      });
    }
   ...
   */
  static async cleanupDeadConnections(): Promise<number> {
    if (this._adapter) {
      return await this._adapter.cleanup();
    }
