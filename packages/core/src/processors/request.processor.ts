...
import { VoltRealtimeService } from "../services/realtime.service";
import { VoltConsoleLogger } from "../services/logger.service";
import { VoltPluginManager } from "../services/plugin.service";
import { resolveLogLevel, createLoggerContext } from "../utils/logger";

/**
 * Handles HTTP request processing for the Volt Framework.
...
export class RequestProcessor<
  TRouter extends VoltRouter<any, any, any, any, any>,
  TConfig extends
...
> implements RequestProcessorInterface<TRouter, TConfig> {
  ...
  public router: RouterContext<
    VoltAction<any, any, any, any, any, any, any, any, any, any>
  >;
  public pluginManager?: VoltPluginManager<any>;
  ...
  private static get isInteractiveMode() {
    return process.env.VOLT_INTERACTIVE_MODE === "true";
  }
  ...
  constructor(config: TConfig) {
    ...
    this.logger = VoltConsoleLogger.create({
      level: process.env.VOLT_LOG_LEVEL as VoltLogLevel || VoltLogLevel.INFO,
      context: {
    ...
    this.router = createRouter<VoltAction<any, any, any, any, any, any, any, any, any, any>>();
  ...
  private initializePluginManager(): void {
    if (this.config.plugins && Object.keys(this.config.plugins).length > 0) {
      try {
        ...
        this.pluginManager = new VoltPluginManager({
          store,
          logger,
          config: {
  ...
  private registerRoutes(): void {
    const basePATH = this.config.basePATH || process.env.VOLT_APP_BASE_PATH || "/api/v1";
    ...
    for (const controller of Object.values(
      this.config.controllers,
    ) as VoltControllerConfig<any>[]) {
      for (const endpoint of Object.values(controller.actions) as VoltAction<
        any,
        any,
        any,
        any,
    ...
    const sseAction: VoltAction<any, any, any, any, any, any, any, any, any, any> = {
      method: "GET",
      type: "query",
      path: "/sse/events",
  ...
  private registerPluginRoutes(basePATH: string): void {
    ...
    for (const pluginName of pluginNames) {
      ...
      for (const [controllerName, controllerActions] of Object.entries(plugin.$controllers)) {
        for (const [actionName, actionConfig] of Object.entries(controllerActions as any) as [string, VoltAction<any, any, any, any, any, any, any, any, any, any>][]) {
          try {
            ...
            const wrappedAction: VoltAction<any, any, any, any, any, any, any, any, any, any> = {
              ...actionConfig,
              ...
              // Ensure required VoltAction properties
  ...
  async process(request: Request): Promise<Response> {
    ...
    try {
      ...
      const basePATH =
        this.config.basePATH || process.env.VOLT_APP_BASE_PATH || "/api/v1";
      const sseEndpoint = parseURL(basePATH, "/sse/events");
      ...
      if (handler.use && Array.isArray(handler.use)) {
        const actionResult = await MiddlewareExecutorProcessor.executeAction(
          context,
          handler.use as VoltProcedure<any, any, any>[],
        );
  ...
  private async executeAction(
    handler: VoltAction<any, any, any, any, any, any, any, any, any, any>,
    ...
    const responseProcessor = VoltResponseProcessor.init(
      context.$plugins?.store || context.$context?.store,
    );

    const realtimeService = new VoltRealtimeService(
      context.$plugins?.store || context.$context?.store
    );

    // Execute handler with proper VoltActionContext structure
  ...
  private async handleSuccessfulResponse(
    ...
    if (actionResponse instanceof VoltResponseProcessor) {
      this.logger.debug('Response processor returned', {
        type: 'VoltResponseProcessor'
      });
   ...
   *
   * @throws {VoltError} When controller or action not found
   */
  async call<
  ...
  >(
    ...
    const basePATH =
      this.config.basePATH || process.env.VOLT_APP_PATH || "/api/v1";
    const baseURL =
      this.config.baseURL ||
      process.env.VOLT_APP_URL ||
      "http://localhost:3000";
   ...
   * @returns Number of clients the event was sent to
   */
  public publishSystemEvent(type: string, data: any): Promise<number> {
   ...
   */
  public revalidateQueries(queryKeys: string | string[]): Promise<number> {
  ...
  static async process(context: any): Promise<any> {
    ...
    try {
      ...
      const deprecationLogger = VoltConsoleLogger.create({
      ...
      error = err as Error;
      statusCode = error instanceof VoltError ? 500 : 500;
      throw error;
    }
  }
}
