import type { ContextCallback, VoltProcedure } from "../types";
...
> = {
  ...
  handler: (params: {
    ...
    response: VoltResponseProcessor<TContext, any>;
  }) => any | Promise<any>;
};
 ...
 */
export function createVoltPluginAction<
  TContext extends object | ContextCallback,
  TName extends string,
  TInput extends StandardSchemaV1,
  TOutput,
 ...
 */
export function createVoltPluginEventListener<
 ...
 */
export function createVoltPluginEventEmitter<
  const TEventName extends string,
  TPayload extends StandardSchemaV1,
>(config: {
  event: TEventName;
...
> =
  {
    global?: VoltProcedure<TContext, any, any>[];
    routes?: Record<RoutePattern, VoltProcedure<TContext, any, any>[]>;
  };
 ...
 * @example
 * ```typescript
 * const authPlugin = createVoltPlugin({
 *   name: 'auth',
 *   $actions: {
 *     validateToken: createVoltPluginAction({ ... }),
 *     createSession: createVoltPluginAction({ ... })
 *   },
 *   $controllers: (actions) => ({
 *     login: {
 *       handler: async ({ self }) => {
 ...
 * ```
 */
export function createVoltPlugin<
  TContext extends object | ContextCallback,
...
export type InferPluginActions<T> =
  T extends VoltPlugin<any, any, any, any, any, infer TActions, any, any>
  ? TActions
  : never;
...
export type InferPluginContext<T> =
  T extends VoltPlugin<infer TContext, any, any, any, any, any, any, any>
  ? TContext
  : never;
...
export type InferPluginContextExtensions<TPlugins extends Record<string, any>> =
  {
    [K in keyof TPlugins]: TPlugins[K] extends VoltPlugin<
      any,
      any,
      any,
      any,
 ...
 * ```
 */
export type InferVoltPlugins<TPlugins extends Record<string, any>> =
