...
import type { HTTPMethod, VoltCookies, VoltHeaders } from "./action.interface";
...
/**
 * Represents the context for an Volt procedure.
 * @template TActionContext - The type of the custom action context.
 * 
 * @typedef {Object} VoltProcedureContext
 ...
 * @property {VoltHeaders} request.headers - The request headers.
 * @property {VoltCookies} request.cookies - The request cookies.
 * @property {TActionContext} context - Custom context passed to the procedure.
 * @property {VoltResponseProcessor} response - The response processor for handling the procedure output.
 * 
 * @since 1.0.0
 */
export type VoltProcedureContext<TActionContext> = {
  request: {
    path: string;
    params: any;
    body: any;
  ...
  response: VoltResponseProcessor<TActionContext, unknown>;
  /**
...
/**
 * Represents a procedure in the Volt framework.
 ...
 */
export type VoltProcedure<
  TActionContext,
  TOptions,
  TOutput
> = {
  name: string;
  handler: (options: TOptions, ctx: VoltProcedureContext<TActionContext>) => Promise<TOutput> | TOutput;
}
 ...
 * ```typescript
 * // Basic usage
 * const loggerProcedure = volt.procedure.simple(async (ctx) => {
...
export interface EnhancedProcedureBuilder<TActionContext> {
   ...
   * @example
   * ```typescript
   * const procedure = volt.procedure()
   ...
   * @example
   * ```typescript
   * const loggerProcedure = volt.procedure()
  ...
  ): VoltProcedure<TActionContext, undefined, TOutput>;
}
...
export interface EnhancedProcedureBuilderWithName<TActionContext, TName extends string> {
  ...
  handler<TOutput>(
    handler: EnhancedProcedureHandler<TActionContext, undefined, TOutput>
  ): VoltProcedure<TActionContext, undefined, TOutput> & { name: TName };
}
...
export interface EnhancedProcedureFactories<TActionContext> {
   ...
   * @example
   * ```typescript
   * const requestLogger = volt.procedure.simple(async ({ request, context }) => {
   ...
   * @example
   * ```typescript
   * const authProcedure = volt.procedure.withSchema({
   *   optionsSchema: z.object({
   *     required: z.boolean().default(false),
   *     roles: z.array(z.string()).optional(),
   ...
   * @example
   * ```typescript
   * const rateLimitProcedure = volt.procedure.fromConfig({
   *   name: 'rate-limiter',
   *   optionsSchema: z.object({
   *     max: z.number().min(1).max(1000).default(100),
   *     windowMs: z.number().min(1000).default(60000),
 ...
 */
type ExtractProcedureOutput<T> = T extends VoltProcedure<any, any, infer TOutput> ? TOutput : never;

/**
 * Infers the procedure context type from an array of VoltProcedure instances.
 * 
 * @typeParam TActionProcedures - An array of VoltProcedure instances with generic types for input, output, and context
 * @returns A union-to-intersection type of the output types of all procedures
 * 
 * @example
 * ```typescript
 * type Procedures = [
 *   VoltProcedure<any, any, { user: User }>,
 *   VoltProcedure<any, any, { requestId: string }>
 * ]
 * type Context = InferProcedureContext<Procedures> // { user: User } & { requestId: string }
 * ```
 */
export type InferProcedureContext<TActionProcedures extends readonly VoltProcedure<any, any, any>[]> =
  TActionProcedures extends readonly []
  ? {}
  : UnionToIntersection<
    TActionProcedures extends readonly (infer TProcedure)[]
    ? TProcedure extends VoltProcedure<any, any, infer TOutput>
...
/**
 * Infers the context type from an array of VoltProcedure instances.
 */
export type InferActionProcedureContext<TActionProcedures extends readonly VoltProcedure<any, any, any>[]> =
  TActionProcedures extends readonly []
  ? {}
  : UnionToIntersection<
    TActionProcedures extends readonly (infer TProcedure)[]
