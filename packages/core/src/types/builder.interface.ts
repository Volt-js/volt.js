...
/**
 * Base configuration interface for the Volt Framework.
 * Defines the base configuration options that can be applied to an Volt application instance.
 */
export type VoltBaseConfig = {
  baseURL?: string;
  basePATH?: string;
}
...
/**
 * Configuration interface for the Volt Framework builder setup.
 * Defines all the core adapters and configuration options that can be applied
 * to an Volt application instance.
 *
 ...
 * ```typescript
 * // Basic configuration
 * type BasicConfig = VoltBuilderConfig<
 *   { db: Database },
 *   RedisStoreAdapter,
 *   ConsoleLogger,
 *   BullMQAdapter,
 ...
 * type CallbackConfig = VoltBuilderConfig<
 ...
 * ```
 */
export interface VoltBuilderConfig<
  TContext extends object | ContextCallback,
  TConfig extends VoltBaseConfig,
  TStore extends VoltStoreAdapter = any,
  TLogger extends VoltLogger = any,
  TJobs extends JobsNamespaceProxy<TContext> = any,
  TTelemetry extends VoltTelemetryProvider = any,
  TPlugins extends Record<string, VoltPlugin<any, any, any, any, any, any, any, any>> = Record<string, any>,
...
> {
   ...
   * ```
   */
  middleware?: readonly VoltProcedure<any, any, any>[];
   ...
   * ```
   */
  store: TStore extends VoltStoreAdapter ? TStore : never;
   ...
   * ```
   */
  logger: TLogger extends VoltLogger ? TLogger : never;
   ...
   * ```
   */
  telemetry: TTelemetry extends VoltTelemetryProvider ? TTelemetry : never;
 ...
 * @example
 * ```typescript
 * const withAuth: VoltBuilderExtension<AppContext> = (builder) =>
 *   builder.middleware([authProcedure({ required: true })]);
 *
 * const withRateLimit: VoltBuilderExtension<AppContext> = (builder) =>
 ...
 * ```
 */
export type VoltBuilderExtension<
  ...
  TRealtime extends VoltRealtimeService,
  TPlugins extends Record<string, VoltPlugin<any, any, any, any, any, any, any, any>>,
 ...
 *   .context<AppContext>()
 *   .helpers.compose(
 *     Volt.helpers.withAuth(authMiddleware),
 *     Volt.helpers.withConfig({ baseURL: 'https://api.example.com' }),
 *     Volt.helpers.withMiddlewares([rateLimitMiddleware, corsMiddleware])
 *   )
 *   .create();
 * ```
 */
export type VoltHelpers<
  TContext extends object,
  TConfig extends VoltBaseConfig
> = {
  /**
   ...
   * @example
   * ```typescript
   * const withDatabaseContext = Volt.helpers.withContext(async (req) => ({
   ...
   * @example
   * ```typescript
   * const authProcedure = volt.procedure()
   ...
   *   });
   *
   * const withAuth = Volt.helpers.withAuth(authProcedure({ required: false }));
   * ```
   */
  withAuth: <
    TContext extends object,
   ...
   * ];
   *
   * const withSecurity = Volt.helpers.withMiddlewares(securityMiddlewares);
   * ```
   */
  withMiddlewares: <
    TContext extends object,
    ...
    TDocs extends DocsConfig
  >(
    middlewares: readonly VoltProcedure<TContext, any, any>[],
  ) => (builder: VoltBuilder<
    TContext,
    TConfig,
    TMiddlewares,
   ...
   * @example
   * ```typescript
   * const withApiConfig = Volt.helpers.withConfig({
   ...
   * @example
   * ```typescript
   * const productionConfig = Volt.helpers.compose(
   *   Volt.helpers.withContext(createProductionContext),
   *   Volt.helpers.withAuth(authMiddleware),
   *   Volt.helpers.withConfig({
   *     baseURL: 'https://api.production.com',
   *     basePATH: '/api/v1'
   *   }),
   *   Volt.helpers.withMiddlewares([
   *     rateLimitMiddleware({ max: 1000 }),
   *     corsMiddleware({ origin: ['https://app.production.com'] })
   *   ])
   * );
   *
   * const volt = Volt.context<AppContext>().extend(productionConfig).create();
   * ```
   */
  compose: <
    TContext extends object,
 ...
 * Ensures all plugins implement the VoltPlugin interface correctly.
 *
 * @template TPlugins - Record of plugin name to plugin instance
 *
 * @example
