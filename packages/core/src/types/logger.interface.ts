/**
 * Log levels for the Volt Logger provider.
 * These levels are used to categorize log messages by severity.
 */
export enum VoltLogLevel {
  FATAL = 'fatal',
  ERROR = 'error',
  WARN = 'warn',
  INFO = 'info',
 ...
 */
export interface VoltLoggerOptions {
  level?: VoltLogLevel;
  /**
   ...
   */
  formatter?: (entry: VoltLogEntry) => string;
  /**
...
/**
 * Interface for a robust, extensible logger provider for the Volt framework.
 * 
...
export interface VoltLogger {
   ...
   */
  log(level: VoltLogLevel, message: string, context?: any, error?: Error | unknown): void;
